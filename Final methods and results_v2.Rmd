...
length(unique(bloom2$ID)) #1516 -> 1319, 1371, 1563 (1561)
...
spyears[which.max(spyears$DataYrs),] # 51
spyears[which.min(spyears$DataYrs),] # 1
nrow(spyears[spyears$DataYrs>=10,]) #414 -> 402 -> 300, 346 -> 468 -> 480 -> 472 -> 470
mean(spyears$DataYrs[spyears$DataYrs>=10]) #24.08085

# Now 10 plus years
sp.10plus <- earl.sp.yr[earl.sp.yr$DataYrs>=10,]
length(unique(sp.10plus$ID)) #414 -> 402 -> 300, 480 -> 473 -> 470
...
```{r}
accptIds <- unique(synonyms.plantsyn.accpt$ID)
missingIDs <- setdiff(unique(sp.10plus$ID),accptIds) # None!! Yay!
...
bloom2[bloom2$scientificName == "Carex pyrenaica ssp. pyrenaica",]
bloom2[bloom2$scientificName == "Carex crandallii",] #8592
synonyms[synonyms$ID == 8592,]

synonyms.plantsyn[synonyms.plantsyn$SYNONYMS == "Carex micropoda",] #8591, want 8592 to be 8591

missingIDs #8592
synonyms[synonyms$ID == missingIDs,]
bloom.sp[bloom.sp$ID == missingIDs,]
synonyms.plantsyn[synonyms.plantsyn$ID == missingIDs,] 
synonyms.plantsyn.accpt[synonyms.plantsyn.accpt$ID == missingIDs, ] 
