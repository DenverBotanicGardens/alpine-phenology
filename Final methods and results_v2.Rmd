...
### START HERE!!! did all those fixes to synonyms.plantsyn.accpt should link to that instead of synonyms???
#synonyms is the current USDA plant list that we have at DBG
bloom.synonyms <- merge(bloom2, synonyms, by.x = "scientificName", by.y = "SYNONYMS")
bloom.synonyms.plantsyn.accpt <- 
  merge(bloom2, synonyms.plantsyn.accpt, by.x = "scientificName", by.y = "SYNONYMS")
...
write.table(unique(bloom2[,c("ID","Family.Name","scientificName")]),
            file="P:/hackathon/alpine-phenology/datasets/bloom2_synplaccpt.csv", sep=",")
...
#how many species in dataset
length(unique(bloom2$scientificName)) #1820 -> 1588, 1703, 1918
length(unique(bloom2$ID)) #1516 -> 1319, 1371, 1563 -> crap, when linked to synonyms.plantsyn.accpt have only 1268! should do that link later later later!
...
# Now 10 plus years
sp.10plus <- earl.sp.yr[earl.sp.yr$DataYrs>=10,]
length(unique(sp.10plus$ID)) #414 -> 402 -> 300, 480 -> 473 -> 472 ## START HERE update paper!
