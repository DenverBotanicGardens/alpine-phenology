...
######## do this to bloom2 ###########
bloom2 <- bloom2[bloom2$scientificName != "Carex atrata",]
...
length(unique(bloom2$ID)) #1516 -> 1319, 1371, 1563 (1563)
...
species.per.year[which.max(species.per.year$SpeciesTotal),] #357, 398 -> 593 species total for all specimens 603 (604) in 1998
...
spyears[which.max(spyears$DataYrs),] # 51
spyears[which.min(spyears$DataYrs),] # 1
nrow(spyears[spyears$DataYrs>=10,]) #414 -> 402 -> 300, 346 -> 468 -> 480 -> 472
mean(spyears$DataYrs[spyears$DataYrs>=10]) #24.03602

# Now 10 plus years
sp.10plus <- earl.sp.yr[earl.sp.yr$DataYrs>=10,]
length(unique(sp.10plus$ID)) #414 -> 402 -> 300, 480 -> 473 -> 472
...
```{r}
accptIds <- unique(synonyms.plantsyn.accpt$ID)
missingIDs <- setdiff(unique(sp.10plus$ID),accptIds) # what is in sp.10plus that isn't in accptIds
unique(merge.name$scientificName[merge.name$ID %in% grep(paste(missingIDs, collapse="|"),
                                                       merge.name$ID, value=TRUE)])

# want to take out C. atrata, remove 8105
# Carex pyrenaica and C. pyrenaica ssp. pyrenaica shoudl be C. micropoda 
#[1] "Carex atrata"  =8105     "Carex pyrenaica"  =8590   "Carex pyrenaica ssp. pyrenaica" =8592
#[4] "Carex crandallii" = 8592

bloom2[bloom2$scientificName == "Carex crandallii",] #8592
synonyms[synonyms$ID == 8592,]

synonyms.plantsyn[synonyms.plantsyn$SYNONYMS == "Carex micropoda",] #8591, want 8590 and 8592 to both be 8591

#but are they in synonyms.plantsyn??
accptIDs.synpla <- unique(synonyms.plantsyn$ID)
missingIDs.synpla <- setdiff(unique(sp.10plus$ID),accptIDs.synpla)
unique(merge.name$scientificName[merge.name$ID %in% grep(paste(missingIDs.synpla, collapse="|"),
                                                       merge.name$ID, value=TRUE)])

###  [1] "Carex atrata"                           "Carex pyrenaica"                        
###  [3] "Eritrichium aretioides"                 "Eritrichium nanum var. aretioides"      
###  [5] "Eutrema edwardsii"                      "Chlorocrepis tristis"                   
###  [7] "Noccaea montana"                        "Poa nervosa"                            
###  [9] "Salix arctica"                          "Ligularia bigelovii"                    
###  [11] "Aster foliaceus"                       "Symphyotrichum foliaceum var. foliaceum"

# All the names associated with IDs that are in sp.10plus but not in synonyms.plantsyn.accpt... need to have as accepted name
#"Antennaria parvifolia" "Carex micropoda" "Veratrum californicum" "Veratrum tenuipetalum"
#"Carex pyrenaica"-> C. micropoda anyway
#"Carex pyrenaica ssp. pyrenaica" "Carex crandallii" 

missingIDs[1] # 2389 # want to keep Antennaria parvifolia in despite Ackerfield's claim it's not above 3200m. It is in FNA and ITIS
synonyms[synonyms$ID == missingIDs[1],]
synonyms[synonyms$SYNONYMS == "Antennaria parvifolia",]
bloom.sp[bloom.sp$ID == missingIDs[1],]
synonyms.plantsyn[synonyms.plantsyn$ID == missingIDs[1],] 
synonyms.plantsyn.accpt[synonyms.plantsyn.accpt$ID == missingIDs[1], ] 
synonyms.plantsyn.accpt <- rbind(synonyms.plantsyn.accpt, 
                                 synonyms.plantsyn[synonyms.plantsyn$SYNONYMS == "Antennaria parvifolia",])

####################################################################
#### START HERE ### once synonomy (accpted names) are set, then can go through and finish info for paper!!
bloom.sp[bloom.sp$ID == 8105,]


# Not in the list anymore!!! ARRRGGGG 
#synonyms[synonyms$ID == missingIDs[2],] #8591
#synonyms[synonyms$SYNONYMS == "Carex micropoda",]
#bloom.sp[bloom.sp$ID == missingIDs[2],]
#synonyms.plantsyn[synonyms.plantsyn$ID == missingIDs[2],] 
#synonyms.plantsyn.accpt[synonyms.plantsyn.accpt$ID == missingIDs[2], ] 
#synonyms.plantsyn.accpt <- rbind(synonyms.plantsyn.accpt, 
#                                 synonyms.plantsyn[synonyms.plantsyn$SYNONYMS == "Carex micropoda",])

synonyms[synonyms$ID == missingIDs[3],] #46683
synonyms[synonyms$SYNONYMS == "Veratrum californicum",]
bloom.sp[bloom.sp$ID == missingIDs[3],]
synonyms.plantsyn[synonyms.plantsyn$ID == missingIDs[3],] 
synonyms.plantsyn.accpt[synonyms.plantsyn.accpt$ID == missingIDs[3], ] 
merge.name[merge.name$ID == missingIDs[3],]

merge.name[merge.name$scientificName == "Veratrum tenuipetalum",] 
```
...
species.per.year.10[which.max(species.per.year.10$SpeciesTotal),] # 400 in 1998
mean(species.per.year.10$SpeciesTotal) # 186
...
mean(earcol$Collections) #168.4098 -> 185.9672
...
```{r}
mean(earcol$Collections) # 186.1639
```
...
length(unique(min1950GDD_2$day)) #1137
length(unique(min1950GDD_2$scientificName)) #641
length(unique(min1950GDD_2$ID)) #473
...
nrow(yrspsp[yrspsp$x>=10,]) #308 -> 375 -> 370
...
length(unique(min1950GDD_2_10$ID)) #375 -> 370
```
...
npn.min1950 <- merge(npn.all, merge.name, by.x = "SciName", by.y = "scientificName")
...
# Need to pick accepted name and family names first!
# By family
```{r}
ggplot(merge.name, aes(Avg_Hi, startDayOfYear, colour = Family.Name))+
  geom_point(shape=1)+
  stat_smooth(method="lm",size = 0.5)+
  theme_bw()+
#  labs(title="Earliest Bloom Date by Average High Temperature") +
...
nrow(yr.1950_2[yr.1950_2$p.value>=0.05,]) #416
nrow(yr.1950_2[yr.1950_2$p.value<0.05,]) #57
