...
plantsyn[plantsyn$Scientific.Name == "Carex heteroneura",]
plantsyn[plantsyn$Accepted.Symbol == "CAHE8",]


synonyms.plantsyn.accpt[synonyms.plantsyn.accpt$Scientific.Name %in% synonyms.plantsyn.accpt$SYNONYMS[synonyms$ID == missingIDs[2]],]

plantsyn[plantsyn$Scientific.Name %in% synonyms$SYNONYMS[synonyms$ID == missingIDs[2]],]
...
synonyms[synonyms$SYNONYMS ==  "Eritrichium aretioides" ,] #17900 !!
...
synonyms[synonyms$SYNONYMS ==  "Veratrum tenuipetalum" ,]
...
synonyms[synonyms$SYNONYMS ==  "Ligularia bigelovii" ,]
synonyms$ID[synonyms$SYNONYMS ==  "Ligularia bigelovii"] <- synonyms.plantsyn$ID[synonyms.plantsyn$SYNONYMS == "Senecio bigelovii var. hallii"]
...
write.table(unique(bloom2[,c("ID","Family.Name","scientificName")]),
            file="P:/hackathon/alpine-phenology/datasets/bloom2_170117.csv", sep=",")
...
species.per.year[which.max(species.per.year$SpeciesTotal),] #357, 398 -> 593 species total for all specimens 604 in 1998
...
spyears[which.max(spyears$DataYrs),] # 51
spyears[which.min(spyears$DataYrs),] # 1
nrow(spyears[spyears$DataYrs>=10,]) #414 -> 402 -> 300, 346 -> 468 -> 480 -> 473!!
mean(spyears$DataYrs[spyears$DataYrs>=10]) #23.77708

# Now 10 plus years
sp.10plus <- earl.sp.yr[earl.sp.yr$DataYrs>=10,]
length(unique(sp.10plus$ID)) #414 -> 402 -> 300, 480 -> 473
