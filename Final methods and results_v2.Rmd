...
Retain the herbarium from which the collection was taken - colo was all Colo! shocker
```{r}
...
#Year, should pull the last four digits
colo.sm$Year <- (sub('.*(\\d{4}).*', '\\1', as.character(colo.sm$Date..dd.month.yyyy.)))

#Year as 4 digit 
table(as.character(colo.sm$Date..dd.month.yyyy.))
table(colo.sm$Year)
colo.sm <- colo.sm[nchar(colo.sm$Year)==4,] 
colo.sm <- colo.sm[nchar(colo.sm$Year)==4 & !grepl("^[A-Za-z]+$",colo.sm$Year),]
colo.sm <- colo.sm[colo.sm$Year<2017,] 
table(colo.sm$Year[colo.sm$Year < 1950])
barplot(table(colo.sm$Year))
colo.sm$Year <- as.numeric(colo.sm$Year)

ggplot(colo.sm, aes(Year))+
  geom_bar()+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  scale_x_continuous(limits=c(1860,2015),breaks=seq(1860,2015,by =10))

sum(table(colo.sm$Year[colo.sm$Year<1950])) # 28856
sum(table(colo.sm$Year[colo.sm$Year>=1950])) # 66534
sum(table(colo.sm$Year)) # 95390


#Manuscript draft 10, line 165 - check number of collections before 1950. 
colo.pre1950 <- colo.sm[colo.sm$Year<1950,]
str(colo.pre1950)


ggplot(colo.pre1950, aes(as.numeric(Year)))+
  geom_bar()+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
table(seinet$institutionCode)
which(names(seinet)=="institutionCode")
...
table(sp.10plus$institutionCode)
```
...
#Error? WTF dcbrtccklrdnl?

synonyms$ID[synonyms$SYNONYMS=="Aster foliaceus"]<- #dcbrtccklrdnl
...
```{r}
#Want numbers for all data, not just 467 species  
# Answer, is this a change in colleciton patterns?
#Peak collection period throughout the study

dpi = 1200
tiff("Q:/Research/Projects/alpine-phenology/Fig_ChangeinCollectionpatterns.tiff", width = 84, height = 75, compression="lzw", units = "mm", res=dpi)
...
ggplot(bloom2, aes(as.factor(Year), startDayOfYear))+
  ...
  xlab("Year")+
  theme(axis.text.x = element_text(angle=90, hjust=1))+
#  scale_x_discrete(limits = c(1950,2011), breaks = seq(1950,2010, by=10)) +
  theme(axis.text=element_text(size=7),axis.title=element_text(size=9))

dev.off()

mean(bloom2$startDayOfYear, na.rm = TRUE)
summary(lm(Year~startDayOfYear, data=bloom2))
length(unique(bloom2$ID))
...
match(yr.1950_2$Species[yr.1950_2$p.value<0.05],yr.1950_2$Species[yr.1950_2$p.value>=0.05])
...
setdiff(checksame$Species,yr.1950_2$Species) # where'd the blank one come from!?


### Species, native status, duration, life form, blooming period per species
appendixA <- merge(yr.1950_2, synonyms.plantsyn, by = "ID") 
appendixA <- appendixA[,c(1,10:11,41:47)]
...
mean(rst.npn$Slope[rst.npn$p.value<0.05 & rst.npn$Slope < 0]) # -0.05419008
mean(rst.npn$adj.r.squ[rst.npn$p.value<0.05 & rst.npn$Slope< 0]) # 0.2762789
unique(merge.name.accpt$AUTH.y[which(merge.name.accpt$ID %in% 
...
```{r}
ggplot(minNPNGDDmax, aes(MaxGDD, startDayOfYear))+
  geom_point(shape=1)+
  stat_smooth(method="lm",colour = "dark grey", size = 0.5)+
  theme_bw()+
  ylab("Ordinal Date")+
...
summary(lm(startDayOfYear~MaxGDD, data = minNPNGDDmax))
```

#NPN low earlier
```{r}
ggplot(minNPNGDDmax[which(minNPNGDDmax$ID %in% 
                            rst.npn$ID[rst.npn$p.value<0.05 & rst.npn$Slope < 0]),], 
       aes(MaxGDD, startDayOfYear, colour = AUTH.y))+
  geom_point(shape=1)+
  stat_smooth(method="lm", alpha=0.05)+
  theme_bw()+
  ylab("Ordinal Date")+
  xlab(expression(paste("GDD at collection date (T"[base] == 0, degree*C,")")))+
  theme(legend.position="none")+
  geom_dl(aes(label = AUTH.y), method = list(dl.combine("first.points","last.points"),
                                             ces = 0.75)) 
...
```{r}
npn_0303 <- read.csv("Q:/Research/Projects/alpine-phenology/NPN spp download 20170303/status_intensity_observation_data.csv")

head(npn_0303)
names(npn_0303)
table(npn_0303$Phenophase_Name,npn_0303$Phenophase_Status)
table(npn_0303$Phenophase_Description,npn_0303$Phenophase_Status)
table(npn_0303$Phenophase_Category,npn_0303$Phenophase_Status)

unique(npn_0303$scientificName)
unique(npn_0303$USDA_PLANTS_Symbol)


npn_0303$Ordinal <- as.numeric(format(as.Date(npn_0303$Observation_Date, 
                                              format = "%Y-%m-%d"),"%j"))
npn_0303$Year <- as.numeric(format(as.Date(npn_0303$Observation_Date,
                                           fomat = "%Y-%m-%d"), "%Y"))

# of the 29 species, seems only 25 have data

ggplot(npn_0303[npn_0303$Phenophase_Status == 1,], 
       aes(Year,Ordinal,colour = Phenophase_Category))+
  geom_point()+
  stat_smooth(method = "lm")+
  scale_x_continuous(breaks = c(2008:2017))

summary(lm(Ordinal~Year, data = npn_0303[npn_0303$Phenophase_Status == 1 &
                                    npn_0303$Phenophase_Category == "Fruits",]))

```
Select the earliest collection per species per year for these NPN species
```{r}
npn.earl.sp.yr <- do.call(rbind,
                          lapply(split(npn_0303[npn_0303$Phenophase_Status == 1,],
                                       npn_0303$Common_Name[npn_0303$Phenophase_Status == 1]), function(name){
                            name <- name[order(name$Year),]
                            name$DataYrs <- nrow(name[!duplicated(name$Year) & !is.na(name$Year),])
                            rows <- aggregate(Ordinal~Year, name, FUN=min)
                            out <- merge(name,rows,by=c("Ordinal","Year"))
                            out <- out[order(out$Year),]
                            out <- out[!duplicated(out$Year),]
                            out
                          }))
...
ggplot(npn.earl.sp.yr[npn.earl.sp.yr$Phenophase_Status == 1 &
                        npn.earl.sp.yr$Year < 2017,],aes(Year,Ordinal))+
  geom_point()+
  stat_smooth(method = "lm")+
  scale_x_continuous(breaks = c(2008:2017))


summary(lm(Ordinal~Year, data = npn.earl.sp.yr[npn.earl.sp.yr$Phenophase_Status == 1 &
                                                 npn.earl.sp.yr$Year < 2017,]))
```
