...
Need make sure have right stuff for collections before 1930, exclude wrong dates
```{r}
...
#collections for use in VisTrails
cols4VT <- lapply(1:45, function(x){
  sp1 <- signsps_45[[x]][,c("scientificName","decimalLongitude",
                          "decimalLatitude","year",
                          "minimumElevationInMeters","institutionCode")]
  sp <- sp1[sp1$decimalLatitude < 41.1 & sp1$decimalLatitude > 36.9 &
             sp1$decimalLongitude > -109 & sp1$decimalLongitude < -102,]
  sp <- sp[!is.na(sp$decimalLatitude),]
  sp <- sp[sp$year < 1931 & sp$year > 1800,]
  sp <- sp[!is.na(sp$year),]
  if(nrow(sp) == 0){
    sp <- data.frame(scientificName = unique(sp1$scientificName),decimalLongitude = NA,
                     decimalLatitude = NA,year = NA,
...
#write.table(seqAlPh, file = "Q:/Research/All_Projects_by_Species/aa_Shapefiles_Maps/aa_ArcMap_Projects/Alpine_Phenology_2/Species_SEINet/FieldData_AlPh_over80pre1930.csv", sep=",", row.names=FALSE)
...
pointsabefore <- SpatialPointsDataFrame(FieldData_pre1980[FieldData_pre1980$id == 1,
                                          c("decimalLongitude","decimalLatitude")],
                                          FieldData_pre1980[FieldData_pre1980$id == 1,],
                                          proj4string=CRS("+init=epsg:4326"))

pointsafter <- SpatialPointsDataFrame(FieldData_all[FieldData_all$id == 1,
                                        c("decimalLongitude","decimalLatitude")],
                                        FieldData_all[FieldData_all$id == 1,],
                                        proj4string=CRS("+init=epsg:4326"))

levelplot(applymodels[[1]], margin=FALSE, 
          main = paste(FieldData_pre1980$scientificName[FieldData_pre1980$id == 1][1]),
            col.regions=c("white","red4"),
            colorkey=list(at=seq(0, 2, 1)),
                 labels=list(at=seq(0, 2, 1)))+
  layer(sp.points(pointsabefore, pch=20, cex=0.5, col="lightblue4"))

levelplot(justcurrentmodels[[1]], margin=FALSE,
            col.regions=c("white","midnightblue"),
            colorkey=list(at=seq(0, 2, 1)),
                 labels=list(at=seq(0, 2, 1)))+
...
change.newold <- lapply(overlap10, function(x){
  Early <- (sum(x[]==1)+sum(x[]==3)) * (res(r)[1]*res(r)[2])/1000000 *0.621371 #to miles? 
  Current <- (sum(x[]==2)+sum(x[]==3)) * (res(r)[1]*res(r)[2])/1000000*0.621371 #to miles? 
  Maintained  <- sum(x[]==3) *(res(r)[1] *res(r)[2])/1000000*0.621371 #to miles? 
  Current_Early <- ( sum(x[] == 2) - sum(x[] == 1) ) * (res(r)[1]*res(r)[2])/1000000 *0.621371 #to miles? 
  out <- cbind(Early,Current,Maintained,Current_Early)
})

netchangevector.newold <- do.call(rbind,change.newold)
boxplot(netchangevector.newold,
        names=c("Early climate","Current climate","Maintained","Current - Early"),
        ylab = "Range Size (sq mi)") #square km change -> pixel change
...
min(FieldData_pre1980$year)

plot(models[[59]], col = c("white","red"), 
     main=paste(FieldData_pre1980$scientificName[FieldData_pre1980$id == 59][1]))
plot(applymodels[[59]], col = c("white","blue"), 
     main=paste(FieldData_pre1980$scientificName[FieldData_pre1980$id == 59][1]))
...
habitXsp <- read.csv("Q:/Research/Projects/alpine-phenology/phensum_yr_385.csv")

test.merge <- merge(FieldData_all,habitXsp,by = "ID")
head(test.merge)
...
#Points for first bloom collecitons
ggmap(colorado)+
  geom_point(data=mergename[mergename$Decimal.Longitude< (-104),],
             aes(Decimal.Longitude,Decimal.Latitude),
             pch=20, alpha=0.5, size=3,colour="red")+
  geom_polygon(data=co_county, aes(x=long, y=lat, group = group),
               fill=NA, color="white")+
  scale_x_continuous(limits = c(-109.1,-101.9), expand = c(0, 0)) +
  scale_y_continuous(limits = c(36.9, 41.1), expand = c(0, 0))


#points of all collections
ggmap(colorado)+
  geom_point(data=FieldData_all, aes(decimalLongitude,decimalLatitude),
             pch=20, alpha=0.5, size=3,colour="red")+
  geom_polygon(data=co_county, aes(x=long, y=lat, group = group),
               fill=NA, color="white")+
  scale_x_continuous(limits = c(-109.1,-101.9), expand = c(0, 0)) +
...
#PRISM models are 4kmx4km so that should be the actual resolution

change.currentapply <- lapply(1:87, function(model){
  x<-overlap_applycurrent[[model]]
  net <- (( sum(x[] == 1) - sum(x[] == 2) ) * 4^2) #apply model - current
  net <- data.frame(net, variable = model)
})
nc.ca <- do.call(rbind,change.currentapply) #net change, current and apply
...
nc.ca <- merge(nc.ca,habitXsp,by.x = "variable",by.y = "ID")
nc.ca$GrowthHabitGroup <- "Shrub/Tree"
nc.ca$GrowthHabitGroup[grep("Forb",nc.ca$GrowthHabit)] <- "Forb/herb"
nc.ca$GrowthHabitGroup[grep("Graminoid",nc.ca$GrowthHabit)] <- "Graminoid"

nc.ca$DurationGroup <- "Annual/Short Lived Perennial"
nc.ca$DurationGroup[nc.ca$Duration == "Perennial"] <- "Perennial"
```
...
ggplot(maxentResults, aes(X.Background.points,Training.AUC))+
  geom_point()+
  stat_smooth()+
  theme_bw()

ggplot(maxentResults, aes(X.Training.samples,Training.AUC))+
  geom_point()+
  stat_smooth()+
  theme_bw()

ggplot(maxentResults, 
       aes(Equal.training.sensitivity.and.specificity.cumulative.threshold,Training.AUC))+
  geom_point()+
  stat_smooth(method="lm")+
  theme_bw()


contlong <- melt(maxentResults[,grep("contri",names(maxentResults))])

ggplot(contlong, aes(y=value,x=variable))+
  geom_boxplot()+
  theme_bw()+
...
max(maxentResults$Equal.training.sensitivity.and.specificity.cumulative.threshold)
min(maxentResults$Equal.training.sensitivity.and.specificity.cumulative.threshold)
...
# check how many records are centroids of counties
colorado <- map("county","Colorado")
plot(colorado)

county <- map_data('county')
cocounties <- county[county$region=="colorado",]
ggplot(cocounties, aes(long, lat, group=group))+
  geom_polygon()+
  coord_map()


centers <- aggregate(, data = cocounties)

gCentroid(colorado, byid = FALSE)


table(nc.ca$DurationGroup,nc.ca$Phenology)
anns <- nc.ca[grep("Annual",nc.ca$DurationGroup),]
anns$Species[anns$Phenology=="NoChange"]
anns$Species[anns$Phenology=="SignficantChange"]

perns <- nc.ca[nc.ca$Phenology == "SignficantChange" & nc.ca$DurationGroup == "Perennial",]
pernsno <- nc.ca[nc.ca$Phenology == "NoChange" & nc.ca$DurationGroup == "Perennial",]

pernsno$Species[pernsno$net>0]
...
ggplot(nc.ca, aes(y=net,x=Phenology))+
  geom_boxplot()+
#  geom_point()+
  labs(y = expression(atop("Difference between projected climate model ", paste("and the current evelope (", km^2,")"))), 
       x = "87 species \n 11 significantly changed in phenology \n 76 showed no change in phenology")+
  theme_bw()+
  facet_wrap(~DurationGroup) +
  annotate(geom="rect", xmin=-Inf, xmax=Inf, ymin=0, ymax = Inf, alpha=0.095, fill="red")+
  annotate(geom="rect",xmin=-Inf, xmax=Inf, ymin=-Inf, ymax = 0, alpha=0.095, fill="blue")
```
...
```{r}
elevrasterspast <- paste("Elevation",1:87,sep="")
...
#skip to here
```{r}
elevrasterscurrent <- paste("applyElevation",1:87,sep="")
...
# get the background points too!
#pmp1 <- data.frame(rasterToPoints(past_models_prob[[1]]))
pmp1  <- extract(past_models_prob[[1]], FieldData_all[FieldData_all$id == 1&
                                                        FieldData_all$year<1931,
                                                      c("decimalLongitude","decimalLatitude")])
training <- lapply(Maxent, function(x){
  read.csv(paste("Q:/Research/All_Projects_by_Species/aa_Shapefiles_Maps/aa_ArcMap_Projects/Alpine_Phenology_2/Species_SEINet/",modelrun,x,"/trainingSamples.csv",sep=""))
})
background <- lapply(Maxent, function(x){
  read.csv(paste("Q:/Research/All_Projects_by_Species/aa_Shapefiles_Maps/aa_ArcMap_Projects/Alpine_Phenology_2/Species_SEINet/",modelrun,x,"/backgroundPoints.csv",sep=""))
})
```

Do all 
```{r}
summary.lm.all <- lapply(1:50, function(now){
  ID1 <- rbind(data.frame(training[[now]],InOut=1),data.frame(background[[1]],InOut=0))
  ID1extract <- extract(past_models_prob[[now]], ID1[,c("x","y")])
  ID1.all <- data.frame(ID1,ID1extract)
  out <- lm(InOut~ID1extract, data=ID1.all)
  out
})

summary(summary.lm.all[[7]])
##START HERE! whhich are better matched, by sample size, by any other category? 
```
...
x <- 1
## NEED TO SWITCH TO LEVELPLOT!! THESE WILL BE WRONG!!!
plot(past_models[[x]], col = c("#696969","#ffffff"),
     legend=FALSE,
     xlab="Longitude", ylab="Latitude")
  points(FieldData_all[FieldData_all$id == x & FieldData_all$year < 1980, c("decimalLongitude","decimalLatitude")],
...
bioclims <- lapply(1:87, function(x){
  allID <- unique(FieldData_pre1980$ID[FieldData_pre1980$id == x])
  pntplot <- FieldData_all[FieldData_all$ID ==allID & FieldData_all$year > 1980,
                 c("decimalLongitude","decimalLatitude")]
...
### Haven't made the images folder in the 10rep (really doesn't have 10 reps) folder
```{r}
dpi = 1200
for(i in 1:87){
...
length(unique(bioclimPCA$id[bioclimPCA$PhenSign=="SignChange"])) #11
length(unique(bioclimPCA$id[bioclimPCA$PhenSign=="NoChange"])) #76
...
```{r}
# need to extract all the climate variables for those points
bioclims_past <- lapply(1:87, function(x){
  allID <- unique(FieldData_pre1980$ID[FieldData_pre1980$id == x])
  pntplot <- FieldData_all[FieldData_all$ID ==allID & FieldData_all$year < 1981,
                 c("decimalLongitude","decimalLatitude")]
...
#Error in writing table!
write.table(bioclimPCA_past, 
            file = paste("Q:/Research/All_Projects_by_Species/aa_Shapefiles_Maps/aa_ArcMap_Projects/Alpine_Phenology_2/Species_SEINet/",modelrun,"/bioclimPCA_past.csv", 
            sep=","), row.names=FALSE)
...
```{r}
plots_past <- lapply(1:87, function(x){
...
## 10/6/2017 need to come back and fix this!
...
```{r}
bioclims_pastpre1930 <- lapply(1:87, function(x){
...
climateenv$Climate[climateenv$Time=="Before 1980"] <- "Early"
climateenv$Climate[climateenv$Time=="After 1980"] <- "Current"


ggplot(climateenv,aes(x = bio.6,y = bio.12, colour= Climate))+
  geom_point()+
  geom_errorbarh(aes(xmin = bio.6-bio.6.sd, xmax = bio.6+bio.6.sd), height = 0.2)+
  geom_errorbar(aes(ymin = bio.12-bio.12.sd, ymax = bio.12+bio.12.sd),width = 0.2)+
  theme_bw()+
...
Table1 saved from final product of Alpine phenology manuscript
```{r}
...
ggplot(FD_all_habit, aes(year, minimumElevationInMeters))+
  geom_point(pch=21)+
...
minavg$Climate[minavg$Time=="Pre1980"] <- "Early"
minavg$Climate[minavg$Time=="Post1980"] <- "Current"
ggplot(minavg, aes(Time, elevation..m., colour=Climate))+
  geom_boxplot()+
  facet_wrap(~stat)+
  labs(y="Elevation (m)")+
  theme_bw()
...
# Examine what happens if you only use points before 1930


Only before 1930 with the climate of 1900 to 1930!   
Need make sure have right stuff for collections before 1930, exclude wrong dates
```{r}
...
unique(seqAlPh$id)
length(unique(seqAlPh$ID))

AlPh50 <- FieldData_all[FieldData_all$ID %in% unique(seqAlPh$ID),-length(seqAlPh)]
seqAlPh30 <- transform(AlPh50, id=match(AlPh50$ID, unique(AlPh50$ID)))

write.table(seqAlPh30, file = "Q:/Research/All_Projects_by_Species/aa_Shapefiles_Maps/aa_ArcMap_Projects/Alpine_Phenology_2/Species_SEINet/FieldData_AlPh_over30post1980.csv", sep=",", row.names=FALSE)
...
#Take results from VisTrails and compare bin maps for each past projected to current with points from current
```{r}
modelrun30 <- "PRISM_pre1930_current_beta5_MESS/" #3 folds, more than 30 points
#applymax <- paste("ApplyMAXENT",1:89,"_1", sep="")
#Maxent <- paste("Maxent_",1:89,sep="")
applymax30 <- paste("ApplyMAXENT",1:50,"_1", sep="")
Maxent30 <- paste("Maxent_",1:50,sep="")

models30 <- lapply(Maxent30, function(x){
  raster(paste("Q:/Research/All_Projects_by_Species/aa_Shapefiles_Maps/aa_ArcMap_Projects/Alpine_Phenology_2/Species_SEINet/",modelrun30,x,"/maxent_bin_map.tif",sep=""))
})

applymodels30 <- lapply(applymax30, function(x){
  raster(paste("Q:/Research/All_Projects_by_Species/aa_Shapefiles_Maps/aa_ArcMap_Projects/Alpine_Phenology_2/Species_SEINet/",modelrun30,x,"/maxent_bin_map.tif",sep=""))
})


# Change the furture (current) models to 2 and 0 and keep past as 1 and 0, when added 3=in both, 2=in current only, 1=past only
overlap30 <- lapply(1:length(models30), function(x){
  past <- models30[[x]]
  current <- applymodels30[[x]]
  current[applymodels30[[x]] == 1] <- 2
  out <- past+current
  out
})

## how different for the only before 1930 to only before 1980
length(unique(FieldData_pre1980$ID))
length(unique(seqAlPh30$ID))
length(which(unique(FieldData_pre1980$ID) %in% unique(seqAlPh30$ID))) #Carex siccata lost!
biggerlist <- which(unique(FieldData_pre1980$ID) %in% unique(seqAlPh30$ID))
which(unique(seqAlPh30$ID)==132) #21 from list of 50
smallerlist <- c(1:20,22:50)

overlap80_30 <- lapply(1:49, function(x){
  pre80 <- models[[biggerlist[x]]] #more points with all previous to 1980 will be 1
  pre30 <- models30[[smallerlist[x]]]
  pre30[models30[[x]]==1]<-2 #fewer points will be 2
  out <- pre80+pre30
  out
})

# 1/red = with 1861:1980 data, 2/blue = with 1861:1930 data, 3/purple = agree
plot(overlap80_30[[44]], col = c("white","red","blue","lavender"))
```
...
for(x in 1:49){
  ...
  pointsabefore <- SpatialPointsDataFrame(seqAlPh30[seqAlPh30$id == x,
                                          c("decimalLongitude","decimalLatitude")],
                                          seqAlPh30[seqAlPh30$id == x,],
                                          proj4string=CRS("+init=epsg:4326"))
  
    
  jpeg(paste("Q:/Research/Presentations/External Conference presentations/Natural Areas Conference 2017/",x,"compare30_80.jpg",sep=""), width = 200, height = 150,
       units = "mm", res = 300)
  
  print(levelplot(overlap80_30[[x]], margin=FALSE,
            col.regions=c("white","red4","midnightblue","lavender"),
            colorkey=list(at=seq(0, 4, 1)),
                 labels=list(at=seq(0, 4, 1)))+
    layer(sp.points(pointsafter, pch=20, cex=0.5, col="lightblue4"))+
...
change30_80 <- do.call(rbind,lapply(overlap80_30, function(x){
  pre80 <- sum(x[]==1)
  pre30 <- sum(x[]==2)
  agree <- sum(x[]==3)
  net <- ((sum(x[]==2) -sum(x[]==1)) * (res(r)[1]*res(r)[2])/1000000)
  out <- cbind(pre80,pre30,agree,net)
  out
}))

boxplot(change30_80)
#mostly agree, end up with around zero difference, more times the fewer points gives a larger distribution than more points. 

```


```{r}
JustMaxent30 <- paste("PRISM_justcurrent_beta5/Maxent_",1:87,sep="")

justcurrentmodels30 <- lapply(JustMaxent30, function(x){
  raster(paste("Q:/Research/All_Projects_by_Species/aa_Shapefiles_Maps/aa_ArcMap_Projects/Alpine_Phenology_2/Species_SEINet/",x,"/maxent_bin_map.tif",sep=""))
})

overlap_applycurrent30 <- lapply(1:length(models30), function(x){
  apply <- applymodels30[[x]]
  current <- justcurrentmodels30[[x]]
  current[justcurrentmodels30[[x]]==1]<-2
  out <- apply+current
  out
})
```
