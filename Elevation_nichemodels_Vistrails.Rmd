...
#ids 13:89 did not
x<-13
plot(overlap10[[x]], col = c("white","tomato","yellow2","antiquewhite1"))
...
for(x in 1:89){
  ...
  tiff(paste("Q:/Research/Presentations/External Conference presentations/Natural Areas Conference 2017/",x,"prepostrangemap.tiff",sep=""), width = 940, height = 470, compression="lzw",
     units = "mm", res = 1200)
  plot(overlap10[[x]], col = c("white","tomato","yellow2","antiquewhite1"))
  points(FieldData_all$decimalLongitude[FieldData_all$year>1980 &
                                          FieldData_all$ID == x],
...
yearplot <- ggplot(FD_all, aes(x=decimalLongitude, y=decimalLatitude, frame=year))+
...
#lots of issues with animation : maybe run as administrator and versions changed from "convert.exe" to "magick.exe"
```{r}
