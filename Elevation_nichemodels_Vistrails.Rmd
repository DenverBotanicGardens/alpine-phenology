...
cols4VT <- lapply(1:45, function(x){
  ...
  sp <- sp[sp$year >= 1980 & sp$year < 2017,]
  sp <- sp[!is.na(sp$year),]
  if(nrow(sp) > 79){
...
identical(seqAlPh,FieldData_pre1980) #DAMN!
table(FieldData_pre1980$id) #89 species
table(seqAlPh$id) #87 now that incorrect earlier records removed
...
JustMaxent <- paste("PRISM_justcurrent_beta5_minmaxtemp_precip/Maxent_",1:89,sep="")
...
for(i in 1:89){
  ...
  print(SpatialPointsDataFrame(FieldData_all[FieldData_all$year>1980 &
                                                        FieldData_all$year<2017 &
                                                        FieldData_all$ID == x,
        c("decimalLongitude","decimalLatitude")],
        FieldData_all[FieldData_all$year>1980 &
                        FieldData_all$year<2017 &
                        FieldData_all$ID == x,],
...
for(i in 1:89){
  ...
  jpeg(paste("Q:/Research/Presentations/External Conference presentations/Natural Areas Conference 2017/",i,"justcurrentpostrangemap.jpg",sep=""), width = 200, height = 150,
       units = "mm", res = 300)
  
  x <- unique(FieldData_pre1980$ID[FieldData_pre1980$id == i])
  # Have some data that has years wrong like >2017 and less than 1800 down to 0
  pointsafter <- SpatialPointsDataFrame(FieldData_all[FieldData_all$year>1980 &
                                                        FieldData_all$year<2017 &
                                                        FieldData_all$ID == x,
        c("decimalLongitude","decimalLatitude")],
        FieldData_all[FieldData_all$year>1980 &
  ...
  pointsabefore <- SpatialPointsDataFrame(FieldData_all[FieldData_all$year<1980 &
                                                          FieldData_all$year>1800 &
                                                          FieldData_all$ID == x,
          c("decimalLongitude","decimalLatitude")],
          FieldData_all[FieldData_all$year<1980 &
  ...
  levelplot(overlap_applycurrent[[x]],
    ...
    layer(sp.points(pointsabefore, pch=8, cex=0.5, col="red3"))
...
test_spdf1 <- as(overlap_applycurrent[[1]], "SpatialPixelsDataFrame")
...
gganimate(compareGIF, "Q:/Research/Presentations/External Conference presentations/Natural Areas Conference 2017/apply2current.gif", ani.width = 750, ani.height = 750, interval = 0.75)
...
for(x in 1:89){
  ...
  jpeg(paste("Q:/Research/Presentations/External Conference presentations/Natural Areas Conference 2017/",x,"prepostrangemap.jpg",sep=""), width = 200, height = 150,
       units = "mm", res = 300)
  
  plot(overlap10[[x]], col = c("white","red","blue","lavender"),
  ...
  points(FieldData_all$decimalLongitude[FieldData_all$year>1980 &
         ...
         FieldData_all$decimalLatitude[FieldData_all$year>1980 &
                                         FieldData_all$ID == x],
         pch = 21, cex = .5, col = "lightblue4")
  points(FieldData_all$decimalLongitude[FieldData_all$year<1980 &
                                          FieldData_all$ID == x],
         FieldData_all$decimalLatitude[FieldData_all$year<1980 &
                                         FieldData_all$ID == x],
         pch = 8, cex = .5, col = "red3")
...
otherinfo <- unique(test.merge[,c(8,46:51)])
...
change.currentapply <- lapply(1:89, function(model){
...
ggplot(nc.ca, aes(y=net,x=Phenology))+
  geom_boxplot()+
#  geom_point()+
  labs(y = expression(paste("Change in predicted area (",km^2,")")), 
       x = "89 species \n 12 significantly changed in phenology \n 77 showed no change in phenology")+
  theme_bw()+
