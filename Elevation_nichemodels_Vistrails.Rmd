...
change.newold <- lapply(overlap10, function(x){
  ...
  Lost <- ( sum(x[] == 2) - sum(x[] == 1) ) * (res(r)[1]*res(r)[2])/1000000 *0.621371 #to miles? 
  out <- cbind(Early,Current,Maintained,Lost)
})
...
```
 ...
 ### START HERE!! don't know if I trust this figure!!
...
#  geom_point()+
  ...
  geom_rect(aes(xmin=-Inf, xmax=Inf, ymin=0, ymax = Inf), alpha=0.1, fill="red")+
  geom_rect(aes(xmin=-Inf, xmax=Inf, ymin=-Inf, ymax = 0), alpha=0.1, fill="blue")
```
