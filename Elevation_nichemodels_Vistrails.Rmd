...
netchange <- lapply(overlap10, function(x){
  net <- (((sum(x[]==2)+sum(x[]==3))-sum(x[]==1))*(res(r)[1]*res(r)[2]))
  net
})

netchangevector <- do.call(rbind,netchange) # square meters
netchangevector <- netchangevector/1000000 # square km
...
<https://stackoverflow.com/questions/42425530/calculate-selected-area-from-stacked-raster-object> 
Attempt #2: 
   1. get number of pixels - sum(x[] == 1) or whatever >= something....    
   2. reproject raster to projected coordinate system UTM   
       a. stk <- projectRaster(from=stk, crs="+proj=utm +zone=13 +north +ellps=WGS84 +datum=WGS84 +units=m +no_defs")    
       b. number of pixels * (res(stk)[1] * res(stk)[2])) (in meters no longer square)
```{r}

modelprojecttest <- projectRaster(models[[1]], r)
test <- projectRaster(models[[1]], 
                      crs = "+proj=utm +zone=13 +ellps=GRS80 +datum=NAD83 +units=m +no_defs" )

change.newold1 <- lapply(overlap10, function(x){
  x <- projectRaster(from=x, crs="+proj=utm +zone=13 +ellps=WGS84 +datum=WGS84 +units=m +no_defs")
  net <- (sum(x[]==2)-sum(x[]==1)) * (res(x)[1]*res(x)[2]) 
  net
})

netchangevector.newold1 <- do.call(rbind,change.newold1)



boxplot(netchangevector.newold1)
hist(netchangevector.newold1)
```
...
plot(overlap10[[59]], col = c("white","red","blue","purple"))

# New and maintained minus old - which is a strange calculation, want the next
...
saveGIF({
  plot(overlap10[[1]], col = c("white","tomato","yellow2","antiquewhite1"))
  for(i in unique(sort(FD_all$year))){
    points(FD_all$decimalLongitude[FD_all$year>1980&FD_all$ID==1],
           FD_all$decimalLatitude[FD_all$year>1980&FD_all$ID==1],
           pch = 21, cex = 0.5, col = "orange")
    points(FD_all$decimalLongitude[FD_all$year<1980&FD_all$ID == 1],
           FD_all$decimalLatitude[FD_all$year<1980&FD_all$ID == 1],
           pch = 8, cex = 0.5, col = "red3")
    }
#  , movie.name = "P:/hackathon/alpine-phenology/animation.gif"
 #   , interval=c(1, rep(0.1, length(unique(sort(FD_all$year)))), 1, 5)
  })
...
#              paste("Q:/Research/All_Projects_by_Species/aa_Shapefiles_Maps/aa_ArcMap_Projects/Alpine_Phenology_2/Species_SEINet/PRISM_pre1980_current_beta5/Elevation",x,sep=""))
#})
...
```{r}
tiff("Q:/Research/Presentations/External Conference presentations/Natural Areas Conference 2017/changeavgminmax.tiff", width = 200, height = 75, compression="lzw",
     units = "mm", res = 1200)
...
plots <- lapply(1:89, function(x){
  alph.pca <- princomp(bioclimPCA[bioclimPCA$id==x,-c(1:3)], cor=TRUE)
...
dpi = 1200
for(i in 1:89){
tiff(paste("Q:/Research/All_Projects_by_Species/aa_Shapefiles_Maps/aa_ArcMap_Projects/Alpine_Phenology_2/Species_SEINet/PRISM_pre1980_current_beta5/Images/PCApast",i,".tiff", 
...
dpi = 1200
for(i in 1:89){
tiff(paste("Q:/Research/All_Projects_by_Species/aa_Shapefiles_Maps/aa_ArcMap_Projects/Alpine_Phenology_2/Species_SEINet/PRISM_pre1980_current_beta5/Images/PCA",i,".tiff", 
           sep=""), width = 130, height = 90, compression="lzw", units = "mm", res=dpi)
...
##START HERE   
Maybe need to compare points in and not in the probability map with a different cut off than whatever the bin maps gives?
...
plots_past <- lapply(1:89, function(x){
  ...
  alph.pca <- princomp(bionow[,-c(1:3)],
                       cor=TRUE)
  sumr <- summary(alph.pca)
  alph.load <- loadings(alph.pca)
  alph.pc <- predict(alph.pca)
