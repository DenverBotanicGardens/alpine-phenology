...
asuFams <- rbind(asuFams, c("ROSACEAE","Potentilla sp.","","","Potentilla"))
```
...
nrow(min1950_2[min1950_2$Year > 1980,]) # 7737
...
#Figure 4A
```{r}
Iggplot(min1950GDD_2, aes(Year,HDD))+
  geom_point()+
  stat_smooth(method = "lm")+
  theme_bw()+
...
nrow(yr.1950_2) #547
nrow(yr.1950_2[yr.1950_2$p.value>=0.05,]) #477
...
#foo3 <- merge(fams[,c(2,4)], precip.1950_2[precip.1950_2$p.value<0.05,])
#nrow(foo3)
#write.table(foo3, file = "Q:/Research/Projects/alpine-phenology/phen_sumlowsig_2.csv", sep=",")
