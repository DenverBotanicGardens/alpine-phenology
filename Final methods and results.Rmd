...
## 4a. change the four species, "Dasiphora fruticosa" "Gentiana aquatica"   "Thlaspi alpestre"    "Saxifraga arguta", that when used with auct. non will cause synonomy problems
   ...
   We do have two rows for Noccaea fendleri ssp. glauca (30635)- accepted name for Thlaspi alpestre. 
  
# bloom2 
SKIPPING all but Dasiphora 9/1/216
```{r}
TPL(c("Dasiphora fruticosa","Potentilla fruticosa","Noccaea fendleri ssp. glauca","Thlaspi alpestre","Gentiana aquatica","Gentiana fremontii","Saxifraga arguta","Saxifraga odontoloma"))
...
length(unique(bloom2$scientificName)) #2299 -> 1835 -> 1938
...
mean(sp.yr$Freq[sp.yr$Freq > 0])  #2.83
...
length(unique(sp.10plus$ID)) #491
```
...
length(unique(merge.name$ID)) #487
mean(merge.name$DataYrs[!duplicated(merge.name$ID)]) # 23.73306 years

nrow(merge.name) #11449
...
###START HERE, do I want to change it back to Dasiphora, didn't I just change it from Dasiphora?!
...
mean(earcol$Collections) #189 -> 187.6885
...
##OR start here if the rest seems fine###
...
length(unique(min1950GDD_2_10$scientificName)) #373
...
sort(table(yr.1950_2$Species)) #Still two Noccaea fendleri ssp. glauca!!!

write.table(yr.1950_2, file = "Q:/Research/Projects/alpine-phenology/phensum_yr.csv", sep=",")

nrow(yr.1950_2) #488 -> 487
nrow(yr.1950_2[yr.1950_2$p.value>=0.05,]) #433 -> 432
...
Check for any duplicate plant names
```{r}
sort(table(precip.1950_2$Species))  
