...
merge.name <- merge(accpt,min1950_2, by="ID")
length(unique(min1950_2$ID)) #491
length(unique(merge.name$ID)) #488 (487 - means no more double Hieracium triste?!)
...
IDxSpecies <- merge.name[!duplicated(merge.name$scientificName),c("ID","scientificName")]
foo1 <- data.frame(table(IDxSpecies$ID,IDxSpecies$scientificName))
foo1[foo1$Freq >0,]
nrow(IDxSpecies)
