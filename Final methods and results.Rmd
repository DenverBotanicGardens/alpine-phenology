...
```{r}
#
x<- unique(bloom2$ID)[100]

minJul.dates.2 <- lapply(unique(bloom2$ID), function(x){
  this.species <- bloom2[bloom2$ID == x,]
  these.rows <- aggregate(startDayOfYear~Year, this.species, FUN=min)
  merge(unique(this.species[,c("ID","Year","startDayOfYear")]), these.rows,
