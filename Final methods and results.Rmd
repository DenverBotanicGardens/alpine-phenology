...
#Fresh start
```{r, eval=FALSE}
rm(annppt, avg.p.yr, avg.p.yr, avg.HDD.yrs, avg.p.yr.cc, avg.snowdepth, 
...
length(unique(bloom.o$scientificName)) #1986 -> 1938
length(unique(bloom2$scientificName)) #(2284) 2219
...
synonyms2[synonyms2$SYNONYMS %in%
            grep(paste(intersect(fixbloom2$Replacement,synonyms2$Species[!is.na(synonyms2$SYNONYMS)]),
                       collapse="|"), synonyms2$SYNONYMS,value=TRUE),]
...
fixbloom2[fixbloom2$Wrong %in% grep(paste(intersect(fixbloom2$Wrong,synonyms2$SYNONYMS),
                                          collapse="|"), fixbloom2$Wrong, value=TRUE),]
...
sort(table(foo$Var1[foo$Freq>0]))  #0 - was 16 with auct. non
```

After combining datasets, find species with at least 10 years of data
```{r}
#how many species in dataset
length(unique(bloom2$scientificName)) #1820
length(unique(bloom2$ID)) #1516
...
mean(sp.yr$Freq[sp.yr$Freq > 0])  #2.87
...
nrow(spyears[spyears$DataYrs>=10,]) #414
mean(spyears$DataYrs[spyears$DataYrs>=10]) #24.98068

sp.10plus <- spyears[spyears$DataYrs>=10,]
length(unique(sp.10plus$ID)) #414
```
...
length(unique(tenyearplus$ID[tenyearplus$DataYrs>9])) #414
length(unique(sp.10plus$ID)) #414
length(unique(tenyearplus$scientificName)) #597
length(unique(tenyearplus$ID)) #414
...
length(unique(min1950_2$ID)) #414

## START HERE ####### put into Manuscript draft 6
#Results: 3rd para
