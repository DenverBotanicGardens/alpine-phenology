...
#Fresh start
```{r, eval=FALSE}
rm(accpt, acpted, allcol, allcollections, alltable, blnew, bloom, bloom.o, bloom2, bloomfoo,
   collJ.sp, collJ.max, collmax, colo, colo.bind, colo.sm, colpyear, colpyear.2, dat, demo1, earcol,
   earliestcollection, fams, fixbloom2, fixbloom2.all, foo, foo12, foo3, foo4, foo5, GD, GDD.1981, gdd.bl,
   gdd.sig, gdd.sig.min, gdddatayrs, GH, gtyr, hdd.bl, hdd.f, hdd.foo, HDD.npn, HDD.0, HDD.0.all, 
   HDD.npn.sig, hdd.snow, high.1950, high.npn_2, high.1950_2, linknames, lo.npn, lo.npn.Jul, low.1950,
   low.1950_2, m15, m9, max.swe, Maxpyr, merge.name, min.dates, min.dates.snow, min1950_2,
   min1950GDD_2, min1950GDDmax, minNPNGDD_2, mite, mite.log, mite.log.st, ml.JYC, mpy.df, NAT, npn, 
   npn.all, npn.big, npn.big.m, npn.foo, npn.min1950, npn.only.1950, npn.specimens, npnHigh, 
   npnonly, npnPrecip, npnYear, onename, plantinfo, plantsyn, precip.1950, precip.bl,
   precip.npn, precip.1950_2, precip.sig, precip.npn.sig, pslope, rst, rst.npn, rst1, rsts, 
   se.posno, secant1, secant2, seg.model, seinet, seinet.bind, seinet.sm, sig.in.merge, 
   sig.interaction.JYC, sig.ml.JYC, sig.yr.1950, sig.yr.1950_2, singlefoo, sp.10plus, sp.yr, sp.yr.sp,
   spyears, spyears.snow, synonyms, syns, temps, tenyearplus, toplot, total.p.yr, total.p.yr.cc, 
...
length(unique(tenyearplus$ID[tenyearplus$DataYrs>9])) #491
length(unique(sp.10plus$ID)) # 498 -> 473 -> 491
length(unique(tenyearplus$scientificName)) #725
...
length(unique(merge.name$ID)) #469! (488)
...
accpt <- rbind(accpt,c("Senecio bigelovii var. bigelovii","41494","","",
                       ...
                       "","",""))
accpt <- rbind(accpt,c("Noccaea fendleri ssp. glauca","30638","","",
                       "","Brassicaceae","Perennial","Forb/herb","L48 (N)",
                       "","",""))
...
merge.name <- merge(accpt,min1950_2, by="ID")
length(unique(min1950_2$ID)) #491
length(unique(merge.name$ID)) #488 

#Check that there is only one species collection date per year
singlefoo <- data.frame(table(merge.name$Year,merge.name$ID))
nrow(singlefoo[singlefoo$Freq> 1,]) #26

# for two species, repeated data for the synonym, get rid of the second set of data
merge.name[merge.name$ID == 41494,] #just repeated for what was the synonym. 
merge.name[merge.name$ID == 43988,] #just repeated for what was the synonym. 

#Need to get rid of the second half, !duplicate or something since I now used the accepted names
removerows.41494 <- c(which(merge.name$ID== 41494))[12:22] #11 long
removerows.43988 <- c(which(merge.name$ID == 43988))[16:30]

merge.name <- merge.name[-c(removerows.41494,removerows.43988),]

rm(singlefoo)
singlefoo <- data.frame(table(merge.name$Year,merge.name$ID))
nrow(singlefoo[singlefoo$Freq> 1,]) #0

length(unique(merge.name$ID)) #488, on 6/24/16 now I'm down to 481!
mean(merge.name$DataYrs[!duplicated(merge.name$ID)]) # 23.69467 years

nrow(merge.name) #11454
...
merge.name$scientificName[merge.name$scientificName == "Potentilla fruticosa"] <- "Dasiphora fruticosa"
merge.name$Growth.Habit[merge.name$scientificName == "Dasiphora fruticosa"] <- "Shrub"
merge.name$Family[merge.name$scientificName == "Dasiphora fruticosa"] <- "Rosaceae"
merge.name$Duration[merge.name$scientificName == "Dasiphora fruticosa"] <- "Perennial"

merge.name$scientificName[merge.name$scientificName == "Dasiphora floribunda"] <- "Dasiphora fruticosa ssp. floribunda"
merge.name$Growth.Habit[merge.name$scientificName == "Dasiphora fruticosa ssp. floribunda"] <- "Shrub"
merge.name$Family[merge.name$scientificName == "Dasiphora fruticosa ssp. floribunda"] <- "Rosaceae"
merge.name$Duration[merge.name$scientificName == "Dasiphora fruticosa ssp. floribunda"] <- "Perennial"
...
# USDA is where I got A. campestris var. borealis
# Poop, ThePlantList says synonym of Artemisia campestris ssp. borealis
# Snow checklist says synonym of Artemisia campestris var. purshii
# merge.name$scientificName[merge.name$scientificName == "Artemisia borealis"] <- "Artemisia campestris var. borealis"
# now going to swap to Artemisia borealis
# merge.name$scientificName[merge.name$scientificName == "Artemisia campestris var. borealis"] <- "Artemisia campestris var. purshii"
...
```{r}
merge.name$Growth.Habit.sm[merge.name$Growth.Habit %in% grep("Graminoid", GH$Var1, value=TRUE)] <- "Graminoid"

merge.name$Growth.Habit.sm[merge.name$Growth.Habit %in% grep("Forb", GH$Var1, value=TRUE)] <- "Forb"

merge.name$Growth.Habit.sm[merge.name$Growth.Habit %in% grep("Forb", grep("shrub", GH$Var1, value=TRUE, ignore.case=TRUE), value=TRUE, invert=TRUE)] <- "Shrubs"

merge.name$Growth.Habit.sm[merge.name$Growth.Habit %in% grep("Shrub", grep("Tree", GH$Var1, value=TRUE, ignore.case=TRUE), value=TRUE, invert=TRUE, ignore.case=TRUE)] <- "Trees"
...
sum(  GH$Freq[GH$Var1 %in% grep("Forb", GH$Var1, value=TRUE)]) #349 Forb/herbs
...
sum(  GH$Freq[GH$Var1 %in% grep("Shrub", grep("Tree", GH$Var1, value=TRUE, ignore.case=TRUE), value=TRUE, invert=TRUE, ignore.case=TRUE)]) #11 trees
...
sum(GD$Freq[GH$Var1 %in% grep("Biennial", grep("Annual", GD$Var1, value=TRUE, ignore.case=TRUE), value=TRUE, invert=TRUE)])
...
sum(NAT[NAT$Var1 %in% grep("L48", NAT$Var1, value=TRUE, ignore.case=TRUE), "Freq"])
sum(NAT[NAT$Var1 %in% grep("L48", NAT$Var1, value=TRUE, ignore.case=TRUE, invert = TRUE), "Freq"])
...
mean(earcol$Collections) #189 -> 187.7705
...
Average number of collections over the study period was XX species collected per year
```{r}
...
#Yarg! Can't figure out why it's already in there, the precip and temps
...
segmented.Raw_Precip.year$psi #1988.3
#Fitted data
...
# Now merge GDD with min1950_2 or merge.name as it's now called
```{r}
nrow(merge.name[merge.name$Year > 1980,]) # 7737 -> 7730 -> 7447 -> 7316
...
length(unique(min1950GDD_2$day)) #1152 -> 1147
length(unique(min1950GDD_2$scientificName)) #487
...
length(unique(min1950GDD_2_10$scientificName)) #372
...
ggplot(totalGDDyr, aes(Year,MaxGDD))+
  ...
  stat_smooth(method = "lm")+
  theme_bw()+
  labs(title= expression(paste("Maximum cumulative GDD (T"[base] == 0, degree*C,")")))
#  xlab("Year")+
...
nrow(yr.1950_2) #488
nrow(yr.1950_2[yr.1950_2$p.value>=0.05,]) #433
nrow(yr.1950_2[yr.1950_2$p.value<0.05,]) #55

nrow(yr.1950_2[yr.1950_2$p.value<0.05,])/nrow(yr.1950_2) #0.1127049

#earlier
nrow(yr.1950_2[yr.1950_2$p.value<0.05 & yr.1950_2$Slope<0,]) #54
mean(yr.1950_2$Slope[yr.1950_2$p.value<0.05 & yr.1950_2$Slope<0]) #-0.5575634
mean(yr.1950_2$adj.r.squ[yr.1950_2$p.value<0.05 & yr.1950_2$Slope<0]) #0.2297303
...
Growing Degree Days - GDD at time of collection  
```{r}
...
nrow(rst[rst$p.value<0.05 & rst$Slope >= 0,])  #24
...
nrow(rst) #406 -> 377 (373)
rst[rst$p.value<0.05,]
nrow(rst[rst$p.value<0.05,])

#earlier
nrow(rst[rst$p.value<0.05 & rst$Slope < 0,]) #66
mean(rst$Slope[rst$p.value<0.05 & rst$Slope < 0]) 
mean(rst$adj.r.squ[rst$p.value<0.05 & rst$Slope < 0]) 

#later
nrow(rst[rst$p.value<0.05 & rst$Slope >= 0,]) #1
rst[rst$p.value<0.05 & rst$Slope >= 0,]
mean(rst$Slope[rst$p.value<0.05 & rst$Slope >= 0]) 
mean(rst$adj.r.squ[rst$p.value<0.05 & rst$Slope >= 0]) 
...
#MaxGDD
```{r}
ggplot(min1950GDDmax, aes(startDayOfYear, MaxGDD, group=scientificName))+
  geom_point()+
  geom_smooth(method="lm", level=0, lwd = .5, colour = "black")+
  theme_bw()+
  ylab(expression(GDD))
...
```{r}
ggplot(min1950GDDmax, aes(startDayOfYear, MaxGDD, group=Growth.Habit, colour=Growth.Habit))+
  geom_point()+
  geom_smooth(method="lm", level=0, lwd = .5)+
  theme_bw()+
  ylab(expression(GDD))
