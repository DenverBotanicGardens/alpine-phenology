...
length(unique(bloom.o$scientificName)) #2299 -> 1835
length(unique(bloom.o$ID)) # 1563 grouped by synonyms

# how many years collected per species
sp.yr <- as.data.frame(table(bloom.o$Year,bloom.o$ID))
nrow(sp.yr[sp.yr$Var2=="",])

nrow(sp.yr[sp.yr$Freq>0,]) #14369
mean(sp.yr$Freq[sp.yr$Freq > 0]) 
max(sp.yr$Freq[sp.yr$Freq>0]) #80
tail(sp.yr)

# How many vouchers per species per year
colpyear <- data.frame(table(bloom.o$ID,bloom.o$Year))
...
nrow(spyears[spyears$DataYrs>=10,]) #473
mean(spyears$DataYrs[spyears$DataYrs>=10]) #23.65751
...
```{r}
tenyearplus <- merge(bloom.o,sp.10plus,by = "ID")
length(unique(sp.10plus$ID)) # 498 -> 473
length(unique(tenyearplus$scientificName)) #654
length(unique(tenyearplus$ID)) #473
...
length(unique(min1950_2$ID)) #473
```
...
# The first one isn't the accepted name... 
onename <- bloom.o[!duplicated(bloom.o$ID),]
