...
intersect(fixbloom2$Replacement,synonyms2$SYNONYMS) #will change to appropriate accepted name
intersect(fixbloom2$Wrong,synonyms2$SYNONYMS) # think is 'wrong' but is what it should be: "Dasiphora fruticosa"           "Phacelia sericea var. sericea" "Trifolium brandegei" 
...
## redo and start here! make sure the names changes from bloomfoo are merged back to bloom2 for later#
...
sort(table(foo$Var1[foo$Freq>0]))  #19 again that have two
...
length(unique(bloom2$ID)) #1517

# how many years collected per species
# Year   ID   #collections
sp.yr <- as.data.frame(table(bloom2$Year,bloom2$ID))

nrow(sp.yr[sp.yr$Freq>0,]) #13588
nrow(sp.yr[sp.yr$Freq>1,]) #6754
...
nrow(spyears[spyears$DataYrs>=10,]) #415
mean(spyears$DataYrs[spyears$DataYrs>=10]) #24.96145

sp.10plus <- spyears[spyears$DataYrs>=10,]
length(unique(sp.10plus$ID)) #415
```
...
length(unique(tenyearplus$ID[tenyearplus$DataYrs>9])) #415
length(unique(sp.10plus$ID)) # 498 -> 473 -> 415
length(unique(tenyearplus$scientificName)) #597
length(unique(tenyearplus$ID)) #415
...
minJul.dates.2 <- lapply(unique(tenyearplus$ID), function(x){
  ...
  merge(unique(this.species[,c("ID","Year","startDayOfYear","DataYrs",
                               "scientificName","Family.Name","Authority")]), #does adding these make multiple rows?
...
[1] "Aster foliaceus"                   "Carex maritima"- should remove! "Castilleja rhexifolia"            
[4] "Chlorocrepis tristis"              "Eritrichium aretioides"            "Eritrichium nanum var. aretioides"
[7] "Eutrema edwardsii"                 "Ligularia bigelovii"               "Noccaea montana"                  
[10] "Veratrum californicum"   
```{r}
