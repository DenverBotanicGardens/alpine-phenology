...
bloom2 <- merge(bloomfoo, synonyms[,-3], by.x = "scientificName", by.y = "SYNONYMS")
foo <- data.frame(table(bloom2$scientificName,bloom2$ID))
sort(table(foo$Var1[foo$Freq>0])) #0
...
min1950_2_10plus <- min1950_2[min1950_2$DataYrs>9,]
```
...
accpt <- rbind(accpt,c("Carex micropoda","8591","","",
                       ...
                       "","",""))
accpt <- rbind(accpt,c("Hieracium triste ","22207","","",
...
sum(  GH$Freq[GH$Var1 %in% grep("shrub", GH$Var1, value=TRUE, ignore.case=TRUE) & GH$Var1 %in% grep("Forb", GH$Var1, value=TRUE, invert=TRUE)]) #33 shrubs/subshrubs
...
nrow(merge.name[merge.name$Year > 1980,]) # 7737 -> 7730 -> 7447
