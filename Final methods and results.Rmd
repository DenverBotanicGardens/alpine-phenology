...
length(badnames.o) #293 -> 509 (after linking plantsyn to synonyms (our version of USDA))
...
remainingerrors <- setdiff(badnames.o,fixbloom2$Wrong)
write.table(remainingerrors,"clipboard", sep="\t", row.names=FALSE)
fixbloom3 <- read.csv(path.expand("Q:/Research/Projects/alpine-phenology/SpellingErrorCorrections_v3plus.csv"),
                      na.strings = c("NA", "", " "), colClasses = "character")
```
...
length(unique(bloom2$scientificName)) #1820 -> 1595
length(unique(bloom2$ID)) #1516 -> 1321

# how many years collected per species
# Year   ID   #collections
sp.yr <- as.data.frame(table(bloom2$Year,bloom2$ID))

nrow(sp.yr[sp.yr$Freq>0,]) #13571 -> 13007
nrow(sp.yr[sp.yr$Freq>1,]) #6750 -> 6560
mean(sp.yr$Freq[sp.yr$Freq > 0])  #2.87 -> 2.92
...
## Do I have any duplicate records per ID in a single year?
sp.yr.sp[,1:2] <- sapply(sp.yr.sp[,1:2], function(x) as.numeric(as.character(x)))

sp.yr.sp <- do.call(rbind,
                    lapply(unique(sp.yr.sp$Var2), function(id){
                      oneID <- sp.yr.sp[sp.yr.sp$Var2==id,]
                      oneID <- oneID[order(oneID$Var1),]
                      oneID[!duplicated(oneID$Var1),]
                    })
                    )

#How many species were collected each year? For all specimens
byyr <- split(sp.yr.sp, sp.yr.sp$Var1, drop=TRUE)
species.per.year <- do.call(rbind,
                            lapply(byyr, function(x){
                              data.frame(SpeciesTotal = sum(x$Freq), Year = unique(x$Var1))
                            }))

species.per.year[which.max(species.per.year$SpeciesTotal),]
...
nrow(spyears[spyears$DataYrs>=10,]) #414 -> 404
mean(spyears$DataYrs[spyears$DataYrs>=10]) #25.10891

sp.10plus <- spyears[spyears$DataYrs>=10,]
length(unique(sp.10plus$ID)) #414 -> 404

#How many species were collected each year? For 404 species with 10 or more years of data
sp.yr.sp.10 <- sp.yr.sp[sp.yr.sp$Var2 %in% grep(paste("^",
                                                      paste(unique(sp.10plus$ID), collapse="|"),
                                                       "$", sep=""),
                                                sp.yr.sp$Var2, value=TRUE),]
byyr.10 <- split(sp.yr.sp.10, sp.yr.sp.10$Var1, drop=TRUE)
species.per.year.10 <- do.call(rbind,
                            lapply(byyr.10, function(x){
                              data.frame(SpeciesTotal = sum(x$Freq), Year = unique(x$Var1))
                            }))

species.per.year.10[which.max(species.per.year.10$SpeciesTotal),]
species.per.year[which.max(species.per.year$SpeciesTotal),]
...
length(unique(tenyearplus$ID[tenyearplus$DataYrs>9])) #414 -> 404
length(unique(sp.10plus$ID)) #414 -> 404
length(unique(tenyearplus$scientificName)) #597 -> 571
length(unique(tenyearplus$ID)) #414 -> 404
...
minJul.dates.2 <- lapply(unique(tenyearplus$ID), function(x){
  ...
  out <- merge(unique(this.species[,c("ID","Year","startDayOfYear","DataYrs",
...
length(unique(min1950_2$ID)) #414 -> 404

#confirm no doubles per year
ids <- unique(tenyearplus$ID)
...
allIDs <- unique(synonyms.plantsyn$ID)
length(unique(synonyms.plantsyn$ID)) #5497
synonyms.plantsyn.accpt <- synonyms.plantsyn[synonyms.plantsyn$Synonym.Symbol=="",]
accptIds <- unique(synonyms.plantsyn.accpt$ID)
length(unique(synonyms.plantsyn.accpt$ID)) #5452

missingIDs <- setdiff(allIDs,accptIds)

missingnames <- unique(min1950_2$scientificName[min1950_2$ID %in% grep(paste(missingIDs, collapse="|"),
                                 min1950_2$ID, value=TRUE)])


# Which accepted names aren't one of the names in the 10 plus species?
unique(min1950_2[min1950_2$ID %in% grep(paste("^", paste(missingIDs, collapse="|"), "$", sep=""),
                                 min1950_2$ID, value=TRUE),c("scientificName","ID")])
```
...
accpt <- data.frame(matrix(vector(), 1, 33,
                    ...
                    stringsAsFactors=F)
#Oligosporus groenlandicus  3449
#Artemisia borealis  3449
accpt <- rbind(accpt,c("Artemisia campestris var. purshii",3449,rep(NA, 24),"Asteraceae","Perennial",
           "Forb/herb","L48 (N)","Fall",rep(NA,2)))
#Besseya alpina  5603
accpt <- rbind(accpt,c("Besseya alpina",5603,rep(NA, 24),"Plantaginaceae","Perennial",
           "Forb/herb","L48 (N)","Late Summer",rep(NA,2)))
#Besseya ritteriana  5608
accpt <- rbind(accpt,c("Besseya ritteriana",5608,rep(NA, 24),"Plantaginaceae","Perennial",
           "Forb/herb","L48 (N)","Late Summer",rep(NA,2)))
#Carex chalciolepis  8325
accpt <- rbind(accpt,c("Carex chalciolepis",8325,rep(NA, 24),"Cyperaceae","Perennial",
           "Graminoid","L48 (N)","Fall",rep(NA,2)))
#Carex phaeocephala  8560
accpt <- rbind(accpt,c("Carex phaeocephala",8560,rep(NA, 24),"Cyperaceae","Perennial",
           "Graminoid","L48 (N)","Fall",rep(NA,2)))
#Askellia nana 12586
#Crepis nana 12586
accpt <- rbind(accpt,c("Crepis nana",12586,rep(NA, 24),"Asteraceae","Perennial",
           "Forb/herb","L48 (N)","Fall",rep(NA,2)))
#Mertensia viridis 29023
#Mertensia bakeri 29023
#Mertensia lanceolata var. nivalis 29023
#Mertensia fusiformis 29023
accpt <- rbind(accpt,c("Crepis nana",29023,rep(NA, 24),"Asteraceae","Perennial",
           "Forb/herb","L48 (N)","Fall",rep(NA,2)))
#Poa secunda 35604
#Poa nevadensis 35604
#Poa sandbergii 35604
#Poa secunda ssp. secunda 35604
...
merge.name <- merge(fin.accpt, min1950_2, by="ID") #will keep SYNONYMS

length(unique(merge.name$ID)) #404! yay

merge.name[merge.name$startDayOfYear < 100,]
```
...
```{r}
nrow(merge.name[merge.name$Year > 1980,]) #6499
...
length(unique(min1950GDD_2_10$scientificName)) #308
```
...
nrow(yr.1950_2[yr.1950_2$p.value>=0.05,]) #356
nrow(yr.1950_2[yr.1950_2$p.value<0.05,]) #48

nrow(yr.1950_2[yr.1950_2$p.value<0.05,])/nrow(yr.1950_2) #0.1188119

#earlier
nrow(yr.1950_2[yr.1950_2$p.value<0.05 & yr.1950_2$Slope<0,]) #44
mean(yr.1950_2$Slope[yr.1950_2$p.value<0.05 & yr.1950_2$Slope<0]) #-0.5348264
mean(yr.1950_2$adj.r.squ[yr.1950_2$p.value<0.05 & yr.1950_2$Slope<0]) #0.2298124

#later
nrow(yr.1950_2[yr.1950_2$p.value<0.05 & yr.1950_2$Slope>=0,]) #4
unique(bloom2$AUTH[bloom2$scientificName %in% grep(paste(yr.1950_2$Species[yr.1950_2$p.value<0.05 & yr.1950_2$Slope>=0],
                                        collapse="|"), yr.1950_2$Species, value=TRUE)])
mean(yr.1950_2$Slope[yr.1950_2$p.value<0.05 & yr.1950_2$Slope>=0]) #0.5457558
mean(yr.1950_2$adj.r.squ[yr.1950_2$p.value<0.05 & yr.1950_2$Slope>=0]) #0.2396479
...
#earlier
nrow(low.1950_2[low.1950_2$p.value<0.05 & low.1950_2$Slope<0,]) #67
...
#later
nrow(low.1950_2[low.1950_2$p.value<0.05 & low.1950_2$Slope>=0,]) #3
unique(bloom2$AUTH[bloom2$scientificName %in% 
                     grep(paste(low.1950_2$Species[low.1950_2$p.value<0.05 & low.1950_2$Slope>=0],
...
unique(bloom2$AUTH[bloom2$scientificName %in% 
                     grep(paste(precip.1950_2$Species[precip.1950_2$p.value<0.05 & precip.1950_2$Slope<0],
...
nrow(yr.npn_2[yr.npn_2$p.value<0.05,])/nrow(yr.npn_2) #0.1481481

#earlier
nrow(yr.npn_2[yr.npn_2$p.value<0.05 & yr.npn_2$Slope<0,]) #4
unique(bloom2$AUTH[bloom2$scientificName %in% 
                     grep(paste(yr.npn_2$Species[yr.npn_2$p.value<0.05 & yr.npn_2$Slope<0],
                                collapse="|"), yr.npn_2$Species, value=TRUE)])
mean(yr.npn_2$Slope[yr.npn_2$p.value<0.05 & yr.npn_2$Slope<0]) #-0.5518231
...
nrow(low.npn_2[low.npn_2$p.value<0.05,])/nrow(low.npn_2) #0.2962963
...
#later
nrow(rst[rst$p.value<0.05 & rst$Slope >= 0,]) #0
...
results.npn <- lapply(unique(minNPNGDD_2_max$scientificName), function(x){
  ...
  JYC <- data.frame(cbind(data.frame(t(foo1$coefficients[2,])),
                          ...
                          Fam = unique(minNPNGDD_2_max$Family)))
  colnames(JYC) <- c("Slope", "StErr", "t.value", "p.value", "Intercept", 
                   "F Stat", "numDF","denDF", "Species", "adj.r.squ","N","Fam")
  JYC
  }
  })
...
nrow(rst.npn[rst.npn$p.value<0.05,]) #3
