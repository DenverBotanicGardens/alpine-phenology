...
earl.sp.yr <- do.call(rbind,
                      lapply(split(bloom2, bloom2$ID), function(id){
                        id <- id[order(id$Year),]
                        id$DataYrs <- nrow(id[!duplicated(id$Year),])
                        rows <- aggregate(startDayOfYear~Year, id, FUN=min)
                        out <- merge(unique(id[,c("ID","Year","startDayOfYear","DataYrs",
                                                  "scientificName","Family.Name","Authority")]),
                                     rows, by=c("startDayOfYear","Year"))
                        out[order(out$Year),]
                        out[!duplicated(id$Year),]
                        })
)
...
species.per.year[which.max(species.per.year$SpeciesTotal),] #357
...
spyears[which.max(spyears$DataYrs),] # 27
spyears[which.min(spyears$DataYrs),] # 1
nrow(spyears[spyears$DataYrs>=10,]) #414 -> 402 -> 300
mean(spyears$DataYrs[spyears$DataYrs>=10]) #15.47333

sp.10plus <- spyears[spyears$DataYrs>=10,]
length(unique(sp.10plus$ID)) #414 -> 402 -> 300
```
...
length(unique(tenyearplus$ID[tenyearplus$DataYrs>9])) #414 -> 402 -> 300
length(unique(sp.10plus$ID)) #402
length(unique(tenyearplus$scientificName)) #597 -> 427
length(unique(tenyearplus$ID)) #414 -> 402 -> 300

min1950_2 <- tenyearplus
...
cols2keep <- setdiff(names(min1950_2),names(fin.accpt))
head(min1950_2[,cols2keep])

merge.name <- merge(fin.accpt, min1950_2[,c(cols2keep,"ID")], by="ID") #will keep SYNONYMS

length(unique(merge.name$ID)) #402 -> 300
