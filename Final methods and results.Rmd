...
length(grep("ssp.",synonyms$SYNONYMS)) #6400
length(grep("subsp.",synonyms$SYNONYMS)) #27
...
fixednames <- c()
for(i in bloomfoo$scientificName){
  fixednames <- c(fixednames,
                  if(i %in% grep(paste(fixbloom2.all$Wrong, collapse="|"),i,value=TRUE)){
                    apply(fixbloom2.all,1,function(fixer){
                      if(i==fixer[1]){
                        gsub(i,pattern=fixer[1],replacement=fixer[2])
                      } 
                      })
                    } else {
                        i
...
str(fixednames)
tail(fixednames)
names <- do.call(c,fixednames)
head(fixednames)
nrow(bloomfoo)
```{r}
i <- bloomfoo$scientificName[43608]
((i %in% grep(paste(fixbloom2.all$Wrong, collapse="|"),i,value=TRUE)))
bloomfoo$scientificName[which(bloomfoo$scientificName %in% i)] 
(i == fixbloom2.all[1,1])

bloomfoo <- as.data.frame(sapply(bloomfoo, gsub, pattern = fixbloom2.all[1,1], replacement = fixbloom2.all[1,2]))

for(i in bloomfoo$scientificName){
  if(!is.null(i %in% grep(paste(fixbloom2.all$Wrong, collapse="|"),i,value=TRUE))){
  bloomfoo$scientificName[which(bloomfoo$scientificName %in% i)] <-
    apply(fixbloom2.all,1,function(fixer){
      if(i == fixer[1]){
        fixer[2]
...
```{r}
errorsbloom <- which(bloom2$scientificName %in% grep(paste(fixbloom2.all$Wrong, collapse="|"), bloom2$scientificName,value=TRUE))
fiexedbloomfoo <- which(fixednames %in% grep(paste(fixbloom2.all$Replacement, collapse="|"), fixednames,value=TRUE))
identical(errorsbloom,fiexedbloomfoo)
intersect(errorsbloom,fiexedbloomfoo)
fixednames[intersect(errorsbloom,fiexedbloomfoo)]

length(bloomfoo$scientificName)

setdiff(fixn, bloomfoo$scientificName)

bloomfoo$scientificName <- fixn
  
head(fixn)
head(bloomfoo)
head(bloom2)
  
nrow(unique(bloom2[,c("ID","scientificName")]))
nrow(unique(synonyms[,c("ID","SYNONYMS")]))

length(unique(synonyms[,c("ID")]))


length(unique(bloom2$ID)) #1563
nrow(unique(bloom2[,c("ID","scientificName")]))
length(unique(bloom2$ID))

idyr <- lapply(unique(bloom2$Year),function(yr){
  data.frame(table(bloom2$scientificName[bloom2$Year==yr],bloom2$ID[bloom2$Year==yr]))
})
idyr[[1]][idyr[[1]]$Freq > 0,]
```
...
length(unique(bloom2$scientificName)) #2294 -> 1828
