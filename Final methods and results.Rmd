...
colo.sm$Year[nchar(colo.sm$Year)!=4 & grepl("^[A-Za-z]+$",colo.sm$Year)]
colo.sm$Year[nchar(colo.sm$Year)==4 & grepl("^[A-Za-z]+$",colo.sm$Year)]

colo.sm$Year[(nchar(colo.sm$Year)==4 & grepl("^[A-Za-z]+$",colo.sm$Year)) | nchar(colo.sm$Year)!=4]

#these are all the ones to get rid of
table(colo.sm$Year[(nchar(colo.sm$Year)==4 & grepl("^[A-Za-z]+$",colo.sm$Year)) | nchar(colo.sm$Year)!=4  | colo.sm$Year > 2012])
...
```{r}
# bloom2 <- rbind(seinet.bind,colo.bind) This is the problem, if I create it again here then miss other fixes, go back to top!
...
length(unique(bloom.o$scientificName)) #2299 -> 1835 -> 1934
length(unique(bloom.o$ID)) # 1563 grouped by synonyms -> 1575

# how many years collected per species
sp.yr <- as.data.frame(table(bloom.o$Year,bloom.o$ID))
nrow(sp.yr[sp.yr$Var2=="",])

nrow(sp.yr[sp.yr$Freq>0,]) #14369 -> 14807
...
nrow(spyears[spyears$DataYrs>=10,]) #473 -> 488
mean(spyears$DataYrs[spyears$DataYrs>=10]) #23.78484
...
length(unique(sp.10plus$ID)) # 498 -> 473 -> 488
length(unique(tenyearplus$scientificName)) #654 -> 725
length(unique(tenyearplus$ID)) #473 -> 488
...
length(unique(min1950_2$ID)) #473 -> 488
```
...
merge.1 <- merge(bloom.o[,-c(4:8)],plantsyn, by.x = "scientificName", by.y = "Scientific.Name", all.x =TRUE)
head(merge.1)
...
length(unique(onename$ID)) #1216 -> 1269

min1950_2.foo <- merge(min1950_2, onename, by="ID", all.x = TRUE)
head(min1950_2.foo)
length(unique(min1950_2.foo$ID)) #498! Yay! -> 473, but what does that mean for missing names? 
...
length(unique(min1950_2$ID)) #473 species -> 488
mean(min1950_2$DataYrs[!duplicated(min1950_2$ID)]) #23.78484 years

nrow(min1950_2) #11083 -> 11498
...
###START HERE#### need to fix min1950_2 merged, got x and y when shouldn't be there!
```{r}
