...
```{r, eval=FALSE}
rm(list= setdiff(ls(), c("avgTemps.max","avgTemps.min","tmps","HDD","HDD.all")))
```
...
length(badnames.o) #293 -> 505 (after linking plantsyn to synonyms (our version of USDA))
...
length(unique(bloom2$scientificName)) #1820 -> 1588
length(unique(bloom2$ID)) #1516 -> 1319
```
...
nrow(sp.yr[sp.yr$Freq>0,]) #13571 -> 12892
nrow(sp.yr[sp.yr$Freq>1,]) #6750 -> 6451
mean(sp.yr$Freq[sp.yr$Freq > 0])  #2.89
max(sp.yr$Freq[sp.yr$Freq>0]) #79
...
nrow(spyears[spyears$DataYrs>=10,]) #414 -> 402
mean(spyears$DataYrs[spyears$DataYrs>=10]) #24.97761

sp.10plus <- spyears[spyears$DataYrs>=10,]
length(unique(sp.10plus$ID)) #414 -> 402
...
length(unique(tenyearplus$ID[tenyearplus$DataYrs>9])) #414 -> 402
length(unique(sp.10plus$ID)) #402
length(unique(tenyearplus$scientificName)) #597 -> 564Γ
length(unique(tenyearplus$ID)) #414 -> 402
...
length(unique(merge.name$ID)) #402
...
mean(earcol$Collections) #168.4098
...
Average number of collections over the study period was 168.4098 species collected per year
```{r}
...
ggplot(unbloom, aes(Year,Avg_Hi))+
  ...
  theme_bw()+
  xlab("Year")+
  ylab(expression("Average High Temperature"~degree*C)) + #ylab(expression("Average high temperature"~degree~C))
  scale_x_continuous(breaks = seq(1950,2011,10))+
...
length(unique(min1950GDD_2_10$scientificName)) #306
```
...
nrow(yr.1950_2[yr.1950_2$p.value>=0.05,]) #354
...
merge.name[merge.name$scientificName == "Pulsatilla patens ssp. multifida",
           c("Year","ID","scientificName",
                                                              "startDayOfYear")]
...
ggplot(merge.name[merge.name$scientificName %in% 
                    grep(paste("^", paste(yr.1950_2$Species[yr.1950_2$p.value<0.05 &
                                                              yr.1950_2$Slope>=0],
                                          collapse="|"), "$", sep=""),yr.1950_2$Species,
                         value=TRUE),], 
       aes(Year, startDayOfYear, colour = scientificName))+
  geom_point(shape=1)+
