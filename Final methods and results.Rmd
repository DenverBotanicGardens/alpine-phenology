...
which(bloomfoo[fixbloom2.all$Wrong %in% bloomfoo$scientificName,])

#(bloomfoo$scientificName[10] %in% grep(paste(fixbloom2.all$Wrong, collapse="|"),bloomfoo$scientificName[10],value=TRUE))

fixednames <- lapply(bloomfoo$scientificName, function(tofix){
  if(tofix %in% grep(paste(fixbloom2.all$Wrong, collapse="|"),tofix,value=TRUE)){
    do.call(rbind,
            apply(fixbloom2.all, 1, function(fixer){
              if(tofix == fixer[1]){
                gsub(tofix,pattern = fixer[1],replacement = fixer[2])
                } else {
                  tofix
                  }
              }))
...
tail(fixednames)
names <- do.call(rbind,fixednames)

which(bloomfoo$scientificName %in% )
