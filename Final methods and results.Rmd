...
synonyms.plantsyn$SYNONYMS <- multisub(synonyms2$Species,synonyms2$SYNONYMS,
                                       synonyms.plantsyn$SYNONYMS)
synonyms.plantsyn <- synonyms.plantsyn[!is.na(synonyms.plantsyn),]

allIDs <- unique(synonyms.plantsyn$ID)
length(unique(synonyms.plantsyn$ID)) #5496
synonyms.plantsyn <- synonyms.plantsyn[synonyms.plantsyn$Synonym.Symbol=="",]
accptIds <- unique(synonyms.plantsyn$ID)
length(unique(synonyms.plantsyn$ID)) #5443

############## START HERE ################
missingIDs <- setdiff(allIDs,accptIds)

min1950_2[min1950_2$ID %in% grep(paste(missingIDs, collapse="|"),
                                 min1950_2$ID, value=TRUE),]

### Check if need to add a line with the accepted name for these IDs to keep them in 
```
