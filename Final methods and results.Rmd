...
length(unique(bloom2$scientificName)) #2294
...
nrow(sp.yr[sp.yr$Freq>0,]) #16685 -> 16666
max(sp.yr$Freq[sp.yr$Freq>0])
head(sp.yr)
sp.yr[100:130,]

# How many vouchers per species per year
colpyear <- data.frame(table(bloom2$scientificName,bloom2$Year))
...
length(spyears[spyears$DataYrs>=10,]) #544
mean(spyears$DataYrs[spyears$DataYrs>=10]) #22.18382

spyears$Species <- row.names(spyears)
sp.10plus <- spyears[spyears$DataYrs>=10,]
nrow(sp.10plus) #547 -> 544, after changing all spelling to coerulea from caerulea and Eritrichum to Eritrichium, fixing arabis/boechera synonomy
```
...
length(unique(tenyearplus$scientificName)) #545 -> 544
