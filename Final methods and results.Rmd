...
```{r, eval=FALSE}
rm(avg.snow, 
   annppt, avg.p.yr, avg.p.yr, avg.HDD.yrs, avg.p.yr.cc, avg.snowdepth, 
   avg.swe, climate, low.npn_2, maxsnow, maysnow, maysnow.min, maysnow.sig,
   melt.avg.swe, melt.snow, meltd, min1950GDD_2_10, min1950GDDsecant2,
   min2plot, minNPNGDD_2_max, secant.poly, secant.poly.all, secant.polysnow,
   accpt, allcol, allcollections, bloom, bloom.o, bloom2, bloomfoo,
   collJ.sp, collJ.max, collmax, colo, colo.bind, colo.sm, colpyear, colpyear.2, 
   earcol, header, header2, i, l.lmerTest.private.contrast, 
   precip.1983, precip.npn.sig1, precip.NPN_2, precip.sig.min,
   synonyms2, sps, TPL.bloom2, TPL.merge.names, TPL.badnames,
   unbloom, unbloomGDD, yr.snow,
   highsigsp, lct, lm.1, lm.coef, lm.sum, lmlist,
   matches, minJul.dates.2, mins.foo, month, name, namebloom2, namesID,
   New.sp, npn.IDs, npns, ptest, pr1981, repro, res, res.qt,
   result, results.npn, sday, sample100, samplell, seg.fit,
   earliestcollection, fixbloom2, fixbloom2.all, foo, GD, 
   gdddatayrs, GH, high.npn_2, high.1950_2, linknames, 
   low.1950_2, merge.name, min1950_2,
   min1950GDD_2, min1950GDDmax, minNPNGDD_2, mite, ml.JYC, 
   mpy.df, NAT, names, namesfoo, secant, segmented.Avg_Hi.year,
   segmented.Avg_Lo.year, row, segmented.start.year, size, species, 
   tonumeric, wrongdates, 
   npn.all, npn.big, npn.big.m, npn.foo, npn.min1950,  
   npnonly, npnPrecip, plantsyn, precip.bl,
   precip.npn, precip.1950_2, precip.sig, rst.npn, 
   secant1, secant2, seg.model, seinet, seinet.bind, seinet.sm, sig.in.merge, 
...
plantsyn <- plantsyn[plantsyn$Genera.Binomial.Author %in% 
                       grep(" non ", plantsyn$Genera.Binomial.Author, 
...
bloom.sp <- merge(bloom2, synonyms.plantsyn, by.x = "scientificName", by.y = "SYNONYMS")
foo <- data.frame(table(bloom.sp$scientificName,bloom.sp$ID))
sort(table(foo$Var1[foo$Freq>1]))
rm(foo)
```
...
head(badnames.o)
length(badnames.o) #293 -> 509

intersect(badnames.o, synonyms2$SYNONYMS) #"Trifolium brandegei"; "Phacelia sericea var. sericea"
synonyms2[synonyms2$SYNONYMS %in% 
            grep(paste(intersect(badnames.o, synonyms2$SYNONYMS), collapse="|"),
                 synonyms2$SYNONYMS,value=TRUE),]

######### START HERE # 9/30/2016 checking the names with Ackerfield (now 512 - just looking at the 293 for now)
write.table(badnames.o,"clipboard", sep="\t", row.names=FALSE)

badnames.out <- c("Astragalus multiflorus","Boechera lemmonii var. lemmonii",
                  "Boechera drepanoloba","Boechera lemmonii var. lemmonii",
                  "Boechera pauciflora","Boechera spatifolia","Carex lanuginosa",
                  "Carex oederi","Castilleja rhexifolia","Chondrophylla aquatica",
                  "Clevea hyalina","Draba lanceolata","Epilobium subdentatum",
                  "Gentiana amarella","Gentiana aquatica","Juncus alpinus",
                  "Lychnis apetala","Macronema discoideum","Myosotis alpestris",
                  "Oenothera suffrutescens","Potentilla subgorodkovii",
                  "Pyrola rotundifolia","Saxifraga arguta","Thlaspi alpestre")
...
```{r}
fixbloom2 <- read.csv(path.expand("Q:/Research/Projects/alpine-phenology/SpellingErrorCorrections_v3.csv"),
                      na.strings = c("NA", "", " "), colClasses = "character")

intersect(fixbloom2$Replacement,synonyms2$SYNONYMS) #fixbloom2 will already fix 3
intersect(fixbloom2$Wrong,synonyms2$SYNONYMS) # think is 'wrong' but is what it should be: "Dasiphora fruticosa"           "Phacelia sericea var. sericea" "Trifolium brandegei"  Will get rid of these in 'fixbloom2.v3'!!!

intersect(fixbloom2$Replacement,synonyms2$Species[!is.na(synonyms2$SYNONYMS)]) #will change to wrong, not accepted name, will fix in fixbloom2.v3!!!

intersect(fixbloom2$Wrong,synonyms2$Species) # think is 'wrong' but is what it should be: "Dasiphora fruticosa"           "Phacelia sericea var. sericea" "Trifolium brandegei" 

intersect(intersect(fixbloom2$Wrong,synonyms2$SYNONYMS),bloom2$scientificName)
#remove the three species that will be 'fixed' again with synonym2 via Jennifer Ackerfield/Melissa Islam
#fixbloom2 <- fixbloom2[fixbloom2$Wrong %in% grep(paste(intersect(fixbloom2$Wrong,synonyms2$SYNONYMS),
#                                          collapse="|"), fixbloom2$Wrong, value=TRUE, invert=TRUE),]

# want to remove the NA replacement so can't take it out first!
intersect(bloom2$scientificName,fixbloom2$Wrong[is.na(fixbloom2$Replacement)])

#maybe try a merge - make the new column scientificName be same if no matching, new if matching
bloomfoo <- bloom2

# Want to put NA in and then take out rows with NA for the scientifcName
matches <- apply(fixbloom2,1,function(checknames){
...
## From below - tenyearplus to min1950_2. The following species names aren't in plantsyn (from USDA plants)
#4e
To change all the wrong column names to the 'replacement' column names, use the multisub function
```{r,eval=TRUE}

#Fix with fixbloom2.all
bloomfoo$scientificName <- multisub(fixbloom2$Wrong,fixbloom2$Replacement,bloomfoo$scientificName)
bloomfoo <- bloomfoo[!is.na(bloomfoo$scientificName),]
...
bloom2$scientificName <- multisub(fixbloom2$Wrong,
                                    fixbloom2$Replacement,bloom2$scientificName)
bloom2 <- bloom2[!is.na(bloom2$scientificName),]
```

#### DO I WANT TO DO THIS BEFORE COMBINING?
```{r}
...
length(unique(bloom2$scientificName)) #1820 -> skipped from badnames.o to here 10/3/2016 1651
length(unique(bloom2$ID)) #1516 -> 1339

# how many years collected per species
# Year   ID   #collections
sp.yr <- as.data.frame(table(bloom2$Year,bloom2$ID))

nrow(sp.yr[sp.yr$Freq>0,]) #13571
nrow(sp.yr[sp.yr$Freq>1,]) #6750
mean(sp.yr$Freq[sp.yr$Freq > 0])  #2.87 -> 2.97
...
nrow(spyears[spyears$DataYrs>=10,]) #414 -> 427
mean(spyears$DataYrs[spyears$DataYrs>=10]) #25.37705

sp.10plus <- spyears[spyears$DataYrs>=10,]
length(unique(sp.10plus$ID)) #414 -> 427
```
...
```{r}
tenyearplus <- merge(bloom2,sp.10plus,by = "ID") 
length(unique(tenyearplus$ID[tenyearplus$DataYrs>9])) #414 -> 427
length(unique(sp.10plus$ID)) #414 -> 427
length(unique(tenyearplus$scientificName)) #597 -> 629
length(unique(tenyearplus$ID)) #414 -> 427

# Not earliest, all collections for species with 10+ yrs data
summary(lm(startDayOfYear~Year, data=tenyearplus))
```
...
minJul.dates.2 <- lapply(unique(tenyearplus$ID), function(x){
  ...
  merge(unique(this.species[,c("ID","Year","startDayOfYear","DataYrs",
                               "scientificName","Family.Name","Authority")]),
        these.rows,by=c("startDayOfYear","Year"))
  })

min1950_2 <- do.call(rbind,minJul.dates.2)
head(min1950_2)

length(unique(min1950_2$ID)) #414 -> 427
...
ids <- unique(min1950_2$ID)
checkdoubles <- lapply(ids,function(id){
  oneofem <- min1950_2[min1950_2$ID==id,]
  oneofem[order(oneofem$Year),]
  })

checkdoubles[[2]]

# Get rid of doubles that are caused by different names
min1950_2 <- do.call(rbind,
                     lapply(ids,function(id){
                       oneofem <- min1950_2[min1950_2$ID==id,]
                       oneofem <- oneofem[order(oneofem$Year),]
                       oneofem[!duplicated(oneofem$Year),]
                       })
                     )

rm(singlefoo,ids, checkdoubles)
```

######## START HERE ######## already linked names so need to do it again? No!
...
### START HERE ##### did we keep or lose these? need to figure out again and correct or whatevers 
#After synonymy from Melissa Islam via Jennifer Ackerfield
## NEED THIS BEFORE 10 years data part, These are ones not found in plantsyn (USDA plants list)
"Aster foliaceus"
"Carex maritima"- should remove! 
...
```{r}
# make a from:to table
accpt <- c("Aster foliaceus","Symphyotrichum foliaceum")
accpt <- sapply(accpt, as.character)
names(accpt) <- c("Syn","Accpt")
accpt <- rbind(accpt, c("Carex maritima","Carex incurviformis")) #8348, given 8445, need change name before linked to 8445
#accpt <- rbind(accpt, c("Castilleja rhexifolia","Castilleja rhexiifolia")) # given as 9086 with rhexiifolia, I added 8950 spelled C. rhexifolia Rydb -> edited to 9086
accpt <- rbind(accpt, c("Chlorocrepis tristis","Heiracium triste")) #22207 should == 22205 Hieracium triste
accpt <- rbind(accpt, c("Eritrichium aretioides","Eritrichium nanum var. elongatum")) #17897 Eritrichium == Eritrichum, Weber called it Eritrichium aretioides as distict from European and not just a variety of E. nanum
accpt <- rbind(accpt, c("Eritrichium nanum var. aretioides","Eritrichium nanum var. elongatum"))
accpt <- rbind(accpt, c("Eutrema edwardsii","Eutrema penlandii"))
#accpt <- rbind(accpt, c("Ligularia bigelovii","")) L. bigelovii var. hallii == Senecio bigelovii var. hallii, no just S. bigelovii in Ackerfield so will remove
#accpt <- rbind(accpt, c("Noccaea montana","")) only in Ackerfield as Noccaea fendleri ssp. glauca
row.names(accpt) <- c("8348","22205","17897","1","2","3")
#      accpt <- rbind(accpt, c("Veratrum californicum","")) #in Ackerfield! need it to be in plantsyn

plantsyn[plantsyn$Scientific.Name %in% grep(paste(accpt[,1], collapse="|"),
         plantsyn$Scientific.Name, value=TRUE),]
```
...
#Use the accepted name changes from Melissa via Jennifer Ackerfield to change names in plantsyn
```{r}
plantsyn$Scientific.Name <- multisub(synonyms2$Species,
                                     synonyms2$SYNONYMS,plantsyn$Scientific.Name)
```
...
# Or don't get rid of the not accepted names yet?
linknames <- merge(unique(min1950_2[,c("scientificName","ID")]),
                   plantsyn[,c(1:4,25:31)],
                   by.x = "scientificName", by.y = "Scientific.Name")

merge.name <- merge(linknames,min1950_2, by="ID")
length(unique(min1950_2$ID)) #415
length(unique(merge.name$ID)) #406 - only 406 of the 415 names are in plantsyn. Need to add info for those
...
foo <- data.frame(table(linknames$ID,linknames$scientificName))
namesfoo <- foo[foo$Freq>1,"Var2"]
linknames[linknames$scientificName %in% grep(paste(namesfoo, collapse="|"), linknames$scientificName,
                                             value=TRUE),c(1:6)]

foo <- data.frame(table(merge.name$scientificName,merge.name$ID))
```
...
### One more time, make merge names  SKIPPING 10/3/2016
```{r}
rm(merge.name)
merge.name <- merge(accpt,min1950_2, by="ID")
...
mean(earcol$Collections) #189 -> 166.1148 -> 286.4
...
```{r}
mean(earcol$Collections) # 166.1148 -> 286
```
...
### START HERE AGAIN! BROKE SOMETHING!!! -- somehow have .x and .y from something before
...
```{r}
tiff("Q:/Research/Projects/alpine-phenology/Fig3d.tiff", width = 84, height = 75, compression="lzw", units = "mm", res=dpi)
