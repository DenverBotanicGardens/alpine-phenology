...
remove genus only, 193 records of only genus
```{r}
nrow(bloom2)
nrow(bloom2[(sapply(gregexpr("[[:alpha:]]+",bloom2$scientificName), function(x) sum(x > 0))) != 1,])
nrow(bloom2[(sapply(gregexpr("[[:alpha:]]+",bloom2$scientificName), function(x) sum(x > 0))) == 1,]) #193
...
length(grep("ssp.",bloom2$scientificName)) #1577 (75) -> 1375 after genus removed
...
length(namebloom2) #2284 -> 1960

# remove the auct non (wrong use of name) from synonym list
nrow(bloom2[bloom2$Authority %in% grep("auct. non", bloom2$Authority, value = TRUE),]) #163
...
###START HERE### after removing some of the synonyms that caused troubles later. 
...
synfoo <- data.frame(table(synonyms$SYNONYMS[1001:10000],synonyms$ID[1001:10000]))
sort(table(synfoo$Var1[synfoo$Freq>0]))
synfoo2 <- table(synfoo$Var1[synfoo$Freq>0])
synfoodups <- synfoo2[synfoo2>1]
unique(synonyms[synonyms$SYNONYMS %in% names(synfoodups),])

unique(synonyms[synonyms$SYNONYMS %in% names(dups),])
unique(bloom2[bloom2$ID %in% dupids,c(1,2,8)])

unique(colo$Authority[grep("non", colo$Authority)]) #Desfontaines, non L.
colo[grep("non", colo$Authority),c(1:5)]
...
#Check for synonyms with 
#link by symbol instead of ID - no, no synonyms are listed
###Nope! go back to lumping#####
```{r, eval= TRUE}
...
plantsyn <- plantsyn[plantsyn$Genera.Binomial.Author %in% 
                       grep("auct",plantsyn$Genera.Binomial.Author,value=TRUE,invert = TRUE),]
...
length(unique(bloom.o1$Accepted.Symbol)) #1429
length(unique(bloom.o1$Synonym.Symbol)) #445
```
