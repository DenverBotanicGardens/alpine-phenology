...
sum of snow over all stations per month over the years
```{r}
#average over all stations per year
total.p.yr <- do.call(rbind,lapply(split(snow[,grep("_Snow Depth", names(snow))], 
                                       snow$`Water Year_`), 
...
total.p.yr.cc <- total.p.yr[!rowSums(total.p.yr) == 0,]
total.p.yr.cc
# Not to be confused with snowmelt - this is just reformating the data
melt.snow <- melt(total.p.yr.cc)
head(melt.snow)
...
total.snow <- aggregate(snow[,c(snow.depth)], by=list(snow$`Water Year_`),
                      function(x) sum(x,na.rm=TRUE))
total.snow <- total.snow[!rowSums(total.snow) == 0,]

do.call(rbind,lapply(split(total.snow, total.snow$Group.1), function(x){ 
  apply(x[,-c(1:2)], 2, function(y) sum(y, na.rm = TRUE))
  }
  ))

snow.long <- reshape(total.snow, direction="long",
                     varying=list(names(total.snow[,-c(1:2)])),
                     timevar="Month",
                     times=names(total.snow[,-c(1:2)]),
                     idvar=c("Group.1"),
                     v.names="Snow.Depth")
