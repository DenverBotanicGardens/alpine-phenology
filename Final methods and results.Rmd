...
```{r}
write.table(, 
            file="P:/hackathon/alpine-phenology/datasets/   .csv", sep=",")
...
plantsyn <- read.table("Q:/Research/Projects/alpine-phenology/USDAplants20160510.txt",
                        header=TRUE, sep = ",") # missing those extra ones but including life habit... and indicating which are accepted (to later be double checked according to akerfield.) These would be missing the ones I added to synonyms by hand....
#### START HERE #### this is why I lose some species!
...
foo <- data.frame(table(synonyms.plantsyn$SYNONYMS,synonyms.plantsyn$ID))
head(foo[foo$Freq > 0,])
foo[foo$Freq > 1,] # Great, no species has more than one ID
```
...
length(unique(bloom2$scientificName)) #1820 -> 1588, 1703
length(unique(bloom2$ID)) #1516 -> 1319, 1371
```
...
species.per.year[which.max(species.per.year$SpeciesTotal),] #357, 398 -> 593 species total for all specimens
...
```{r}
mean(earcol$Collections) # 190.7869
```
