...
fixednames <- apply(fixbloom2.all, 1, function(fix){
  gsub(bloomfoo$scientificName,pattern = fix[1],replacement = fix[2])})

```
scinamesfixed <- lapply(bloomfoo$scientificName, function(name){
  if(name %in% fixbloom2.all$Wrong){
    fixbloom2.all$Replacement[grep(name, fixbloom2.all$Wrong)]
  } else {
    name
  }
})
...
```{r}
fixn <- do.call(rbind,scinamesfixed)
length(fixn)
