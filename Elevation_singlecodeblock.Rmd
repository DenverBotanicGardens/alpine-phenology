...
Get bioclim data for all points. Look at shifts in elevation vs. climatic envelope over time for all species.
```{r}
merge.name.accpt_cc <- merge.name.accpt[!is.na(merge.name.accpt$Decimal.Latitude),]

# Center would be  39.113014, -105.358887
#merge.name.accpt_cc <- merge.name.accpt_cc[merge.name.accpt_cc$Decimal.Latitude!=39,] 

# get at least one data point per year to keep species for at least 10 years??? Keep all for now
table(merge.name.accpt_cc$Year, merge.name.accpt_cc$ID)
sp.p.year <- data.frame(table(merge.name.accpt_cc$ID,merge.name.accpt_cc$Year))
sp.p.year.1 <- sp.p.year[sp.p.year$Freq>0,]
sum.p.year <- aggregate(sp.p.year.1$Freq, list(sp.p.year.1$Var2), length)
mean(sum.p.year$x) #112
sum.p.year[which.max(sum.p.year$x),] #2004, 322
rm(sp.p.year,sp.p.year.1)

#How many years have each species been collected?
bysp <- split(merge.name.accpt_cc, merge.name.accpt_cc$ID, drop=TRUE)
...
# 389 species
spyears[which.max(spyears$DataYrs),] # 39
spyears[which.min(spyears$DataYrs),] # 2
nrow(spyears[spyears$DataYrs>=10,]) #318 
mean(spyears$DataYrs[spyears$DataYrs>=10]) #19.90252




#Institutions with data for mapping, one is Lat 26 something - that's out of colorado!
output.herb <- paste(names(table(merge.name.accpt_cc$institutionCode)),"(",
      table(merge.name.accpt_cc$institutionCode),")",sep="")
cat(paste(output.herb, ",", sep=""), "\n")

table(merge.name.accpt_cc$Year[merge.name.accpt_cc$Year > 1980],
      merge.name.accpt_cc$ID[merge.name.accpt_cc$Year > 1980])

# get bioclims from these same collections used to talk about phenology
bioclims.385.current <- lapply(unique(merge.name.accpt_cc$ID), function(x){
  pntplot <- merge.name.accpt_cc[merge.name.accpt_cc$ID ==x & 
                                   merge.name.accpt_cc$Year > 1980,
                                 c("Decimal.Longitude","Decimal.Latitude")]
  out <- merge.name.accpt_cc[merge.name.accpt_cc$ID ==x & 
                                   merge.name.accpt_cc$Year > 1980,]
  for(i in 1:19){
    bioclimnow <- raster(paste("Q:/Research/All_Projects_by_Species/aa_Shapefiles_Maps/aa_ArcMap_Projects/Alpine_Phenology_2/Species_SEINet/prism_1981_2011/bio",i,".tif",
                               sep=""))
    out <- data.frame(out, bio = extract(bioclimnow, pntplot, fun=mean))
    }
  out
  })
bc385c <- do.call(rbind,bioclims.385.current)

shiftnames <- names(bc385c)
names(bc385c) <- c(shiftnames[-grep("bio", shiftnames)[1]],"bio.19") #get rid of the first bio

sort(table(merge.name.accpt_cc$ID[merge.name.accpt_cc$Year < 1981]))

## Get bioclims from 1900:1930
bioclims.385.past <- lapply(unique(merge.name.accpt_cc$ID), function(x){
  pntplot <- merge.name.accpt_cc[merge.name.accpt_cc$ID ==x & 
                                   merge.name.accpt_cc$Year < 1981,
                 c("Decimal.Longitude","Decimal.Latitude")]
    out <- merge.name.accpt_cc[merge.name.accpt_cc$ID ==x & 
                                   merge.name.accpt_cc$Year < 1981,]
  for(i in 1:19){
...
bc385p <- do.call(rbind,bioclims.385.past)
shiftnames <- names(bc385p)
names(bc385p) <- c(shiftnames[-grep("bio", shiftnames)[1]],"bio.19") #get rid of the first bio

write.table(bc385c, file = "Q:/Research/All_Projects_by_Species/aa_Shapefiles_Maps/aa_ArcMap_Projects/Alpine_Phenology_2/Species_SEINet/bioclims385current.csv", sep=",", row.names=FALSE)

write.table(bc385p, file = "Q:/Research/All_Projects_by_Species/aa_Shapefiles_Maps/aa_ArcMap_Projects/Alpine_Phenology_2/Species_SEINet/bioclims385past.csv", sep=",", row.names=FALSE)

```

```{r}
ggplot(merge.name.accpt_cc, aes(Year,elevation))+
  geom_point()+
  stat_smooth(method="lm")

summary(lm(elevation~Year,data=merge.name.accpt))

ggplot(bc385p, aes(Year,elevation))+
  geom_point()+
  stat_smooth(method="lm")

ggplot(bc385c, aes(Year,elevation))+
  geom_point()+
  stat_smooth(method="lm")
...
```{r}
bloom2 <- read.csv(text = getURL("https://raw.githubusercontent.com/DenverBotanicGardens/alpine-phenology/master/datasets/bloom2_170512.csv"))
...
bloom2.latlong <- bloom2[!is.na(bloom2$Decimal.Latitude),]

# get bioclims from these same collections used to talk about phenology

pntplot <- bloom2.latlong[bloom2.latlong$Year > 1980,
                          c("Decimal.Longitude","Decimal.Latitude")]
bloom2.bioclim.current <- bloom2.latlong[bloom2.latlong$Year > 1980,]
for(i in 1:19){
  bioclimnow <- raster(paste("Q:/Research/All_Projects_by_Species/aa_Shapefiles_Maps/aa_ArcMap_Projects/Alpine_Phenology_2/Species_SEINet/prism_1981_2011/bio",i,".tif",
                             sep=""))
  bloom2.bioclim.current <- data.frame(bloom2.bioclim.current, 
                                    bio = extract(bioclimnow, pntplot, fun=mean))
  }

shiftnames <- names(bloom2.bioclim.current)
names(bloom2.bioclim.current) <- c(shiftnames[-grep("bio", shiftnames)[1]],"bio.19") #get rid of the first bio

rm(pntplot)

## Get bioclims from 1900:1930
pntplot <- bloom2.latlong[bloom2.latlong$Year < 1981,
                 c("Decimal.Longitude","Decimal.Latitude")]
bloom2.bioclim.past <- bloom2.latlong[bloom2.latlong$Year < 1981,]
for(i in 1:19){
  bioclimnow <- raster(paste("Q:/Research/All_Projects_by_Species/aa_Shapefiles_Maps/aa_ArcMap_Projects/Alpine_Phenology_2/Species_SEINet/prism_1900_1930/bio",i,".tif",
                             sep=""))
  bloom2.bioclim.past <- data.frame(bloom2.bioclim.past,
                                    bio = extract(bioclimnow, pntplot, fun=mean))
}

shiftnames <- names(bloom2.bioclim.past)
names(bloom2.bioclim.past) <- c(shiftnames[-grep("bio", shiftnames)[1]],"bio.19") #get rid of the first bio
...
ggplot(bloom2.bioclim.past, aes(Year, elevation))+
  geom_point()+
  stat_smooth(method="lm")+
  theme_bw()

summary(lm(elevation~Year,data=bloom2.bioclim.past))
