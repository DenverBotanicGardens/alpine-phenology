...
#Wilcoxon
# ANOVA: parametric equivalent of the Kruskal-Wallis test
anovatest <- lapply(1:89, function(x){
  ...
  pre2post <- matrix(c(zero,one,two,three,zero.p,one.p,two.p,three.p),
                     nrow=2, byrow=TRUE)
  tot <- sum(pre2post)
  
  chiout <- chisq.test(pre2post,simulate.p.value = TRUE, B=10000)
  chiout$expected
  chiout$residuals
  
  anovain <- data.frame(Time = c(rep("Pre",4),rep("Post",4)),
                        Category = c(1:4,1:4),
                        Count = c(zero,one,two,three,zero.p,one.p,two.p,three.p))
  
  anovaout <- lmer(Count ~ Time*Category + (1|Category), REML = FALSE,data = anovain)
  qqnorm(residuals(anovaout))
  qqnorm(anovaout, ~ranef(., level=2))
  plot(density(residuals(anovaout), na.rm=TRUE))
  plot(anovaout)
