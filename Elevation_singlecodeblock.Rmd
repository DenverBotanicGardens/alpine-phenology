...
```{r}
minavg <- rbind(data.frame(value = avgElev2,
                           stat = "avg",
                           Time = "Pre1980"),
                data.frame(value = minElev2,
                           stat = "min",
                           Time = "Pre1980"),
                data.frame(value = maxElev2,
                           stat = "max",
                           Time = "Pre1980"),
                data.frame(value = applyavgElev2,
                           stat = "avg",
                           Time = "Post1980"),
                data.frame(value = applyminElev2,
                            stat = "min",
                            Time = "Post1980"),
                data.frame(value = applymaxElev2,
                            stat = "max",
                            Time = "Post1980")
                )
...
ggplot(minavg, aes(Time, value, colour=Time))+
  geom_boxplot()+
  facet_wrap(~stat)
...
plot(past_models[[1]], col = c("#696969","#ffffff"),
     legend=FALSE)
  points(all[all$id == 1 & all$year < 1980, c("decimalLongitude","decimalLatitude")],
...
```{r}
inout <- do.call(rbind,lapply(bioclims, function(x){
...
ggplot(inout, aes(Value.Var1,Value.Freq,colour = Value.Var1))+
  geom_boxplot()+
