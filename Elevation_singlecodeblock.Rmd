...
#Start here, 1:45 are sign changing phenology, 46:340 (339) are not sign changing in phenology   
```{r}
x <- 89
...
summary(glm(InOut~Comp.1*Comp.2, data=p.df))
# for comp.1
...
pca.results <- lapply(1:89, function(x){
  ...
  p.tt1 <- t.test(Comp.1~InOut, data=loads, paired=FALSE)
  p.tt2 <- t.test(Comp.2~InOut, data=loads, paired=FALSE)
  #Component 1
  outestimate.1 <- p.tt1$estimate[1]
  inestimate.1 <- p.tt1$estimate[2]
  pval.1 <- p.tt1$p.value
  #Component 2
  outestimate.2 <- p.tt2$estimate[1]
  inestimate.2 <- p.tt2$estimate[2]
  pval.2 <- p.tt2$p.value
  
  #Start here! make it a column for each Comp.1 with a corresponding column for in:1 and out:0
  out <- data.frame(id=x, 
                    data.frame(Comp.1=c(outestimate.1,inestimate.1),
                               InOut = c(0,1)),
                    p.val.1=pval.1,
                    data.frame(Comp.2 = c(outestimate.2,inestimate.2),
                               InOut = c(0,1)),
                    p.val.2=pval.2)
  out
})
...
pca.results.all <- do.call(rbind, pca.results)

head(pca.results.all)

#Awesome but comp1 changes for each, need to summarize what's important for comp1 across...
ggplot(pca.results.all, aes(InOut,Comp.1,group=InOut))+
  geom_boxplot()
```
