...
library(list)
filenames <- list.files("Q:/Research/All_Projects_by_Species/aa_Shapefiles_Maps/aa_ArcMap_Projects/Alpine_Phenology_2/Species_SEINet/SignEarl_385", pattern="*.csv")

signsps <- lapply(1:45, function(file){
  read.csv(paste("Q:/Research/All_Projects_by_Species/aa_Shapefiles_Maps/aa_ArcMap_Projects/Alpine_Phenology_2/Species_SEINet/SignEarl_385/",filenames[file], sep=""))
})

filenamestiff <- gsub(".csv", ".tiff", filenames)

for(i in 1:45){
  tiff(paste("Q:/Research/EXTERNAL PARTNERS/NCAR/Elevation_changes/",filenamestiff[i],sep=""), 
       width = 84, height = 75, compression="lzw", units = "mm", res=1200)
  
  print(ggplot(signsps[[i]][signsps[[i]]$year>1850,], aes(year,minimumElevationInMeters))+
          geom_point(shape=1, size=0.5)+
          stat_smooth(method="lm")+
          theme(axis.text=element_text(size=5),
                axis.title=element_text(size=7))+
...
lm_sum <- lapply(signsps, function(sp){
  summary(lm(minimumElevationInMeters~year,data=sp[sp$year>1850,]))
})
...
write.table(all_sum, file = "Q:/Research/EXTERNAL PARTNERS/NCAR/Elevation_changes/elev_change_45.csv",
            sep=",",
            row.names=FALSE)
```
...
signsps <- lapply(1:346, function(file){
...
for(i in 1:346){
  ...
  print(ggplot(signsps[[i]][signsps[[i]]$year>1850 & signsps[[i]]$year<2017 &
                              ...
                              signsps[[i]]$minimumElevationInMeters>0&
...
# Need to remove Ligusticum porterii and Festuca idahoensis (154) for no elevation data

unique(signsps[[154]]$scientificName)
length(signsps[-154])

#Don't know why not working!
unique(signsps[[153]]$scientificName)
table(signsps[[153]]$year)
table(signsps[[153]]$minimumElevationInMeters) 


length(signsps[[153]]$minimumElevationInMeters)
foo<-data.frame(table(signsps[[153]]$minimumElevationInMeters,signsps[[153]]$year))
ggplot(foo[foo$Freq>0,],aes(Var2, Var1))+
  geom_point()


for(i in 1:46){
  print(i)
  print(unique(signsps[[i]]$scientificName))
  print(str(signsps[[i]]$minimumElevationInMeters))
}

lm_sum <- lapply(signsps[1:100], function(sp){
  sp1 <- sp[sp$year>1850 &
              sp$year<2017&
              sp$minimumElevationInMeters>0&
...
summary_tble <- lapply(1:100, function(x){
  sp <- lm_sum[[x]]
  data.frame(filenames[[x]],
             cbind(data.frame(t(sp$coefficients[2,])),
                   sp$coefficients[1,1], #Intercept
...
all_sum <- do.call(rbind,summary_tble)
write.table(all_sum, file = "Q:/Research/EXTERNAL PARTNERS/NCAR/Elevation_changes/elev_change_1_100_non.csv",
            sep=",",
            row.names=FALSE)


lm_sum201 <- lapply(signsps[201:346], function(sp){
  sp1 <- sp[sp$year>1850 &
              sp$year<2017&
              sp$minimumElevationInMeters>0&
              sp$minimumElevationInMeters<4500,]
...
summary_tble201 <- lapply(1:100, function(x){
  sp <- lm_sum201[[x]]
  data.frame(filenames[[x]],
             cbind(data.frame(t(sp$coefficients[2,])),
                   sp$coefficients[1,1], #Intercept
...
all_sum201 <- do.call(rbind,summary_tble201)
write.table(all_sum201, file = "Q:/Research/EXTERNAL PARTNERS/NCAR/Elevation_changes/elev_change_201_346_non.csv",
            sep=",",
            row.names=FALSE)


## final ones without the errored ones
lm_sum101 <- lapply(signsps[101:152], function(sp){
  sp1 <- sp[sp$year>1850 &
              sp$year<2017&
              sp$minimumElevationInMeters>0&
              sp$minimumElevationInMeters<4500,]
...
summary_tble101 <- lapply(1:52, function(x){
  sp <- lm_sum101[[x]]
  data.frame(filenames[[x]],
             cbind(data.frame(t(sp$coefficients[2,])),
                   sp$coefficients[1,1], #Intercept
...
all_sum101 <- do.call(rbind,summary_tble101)
write.table(all_sum101, file = "Q:/Research/EXTERNAL PARTNERS/NCAR/Elevation_changes/elev_change_101_152_non.csv",
            sep=",",
            row.names=FALSE)


## final ones above without the errored ones
lm_sum154 <- lapply(signsps[195:200], function(sp){
  sp1 <- sp[sp$year>1850 &
              sp$year<2017&
              sp$minimumElevationInMeters>0&
              sp$minimumElevationInMeters<4500,]
...
summary_tble154 <- lapply(1:6, function(x){
  sp <- lm_sum154[[x]]
  data.frame(filenames[[x]],
             cbind(data.frame(t(sp$coefficients[2,])),
                   sp$coefficients[1,1], #Intercept
...
all_sum154 <- do.call(rbind,summary_tble154)

####
## final ones above without the errored ones
lm_sumothers <- lapply(signsps[155:193], function(sp){
  sp1 <- sp[sp$year>1850 &
              sp$year<2017&
              sp$minimumElevationInMeters>0&
              sp$minimumElevationInMeters<4500,]
...
summary_tbleothers <- lapply(1:39, function(x){
  sp <- lm_sumothers[[x]]
  data.frame(filenames[[x]],
             cbind(data.frame(t(sp$coefficients[2,])),
                   sp$coefficients[1,1], #Intercept
...
all_sumothers <- do.call(rbind,summary_tbleothers)

all_ALL_sum <- rbind(all_sum, all_sum101, all_sum201, all_sum154,all_sumothers)

write.table(all_ALL_sum, file = "Q:/Research/EXTERNAL PARTNERS/NCAR/Elevation_changes/elev_change_ALL_non.csv",
            sep=",",
            row.names=FALSE)

nrow(all_ALL_sum) #297
mean(all_ALL_sum$Estimate[all_ALL_sum$Pr...t..<0.05 & all_ALL_sum$Estimate<0]) #-2.733852
mean(all_ALL_sum$Estimate[all_ALL_sum$Pr...t..<0.05 & all_ALL_sum$Estimate<0]) #-2.733852
