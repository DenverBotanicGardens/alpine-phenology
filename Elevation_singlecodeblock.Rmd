...
signsps <- lapply(1:339, function(file){
...
signsps_1[[9]][complete.cases(signsps_1[[9]]$decimalLatitude),]
...
cols4VT <- lapply(1:45, function(x){
  ...
  sp <- sp[complete.cases(sp$decimalLatitude),]
  if(nrow(sp) > 79){
...
cols4VT_339 <- lapply(46:384, function(x){
  sp <- signsps[[x-45]][,c("scientificName","decimalLongitude",
                        "decimalLatitude","year","geodeticDatum",
                          "minimumElevationInMeters")]
  sp <- sp[complete.cases(sp$decimalLatitude),]
  if(nrow(sp) > 79){
