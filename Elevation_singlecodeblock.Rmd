...
aggregate(Freq~Var1, inout, sd) # 0: 0.1928782, 1:0.1928782   same?
...
plots_past <- lapply(1:89, function(x){
  alph.pca <- princomp(bioclimPCA_past[bioclimPCA_past$id==x,-c(1:3)], cor=TRUE)
  sumr <- summary(alph.pca)
  alph.load <- loadings(alph.pca)
  alph.pc <- predict(alph.pca)
  
  p <- ggplot(data.frame(alph.pc,bioclimPCA_past[bioclimPCA_past$id==x,1:2]), 
              aes(Comp.1,Comp.2, colour=as.factor(InOut)))+
    geom_point()+
    stat_ellipse()
  list(p,sumr,alph.load,alph.pc)
