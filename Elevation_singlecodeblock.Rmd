...
filenames_1tiff <- gsub(".csv", ".tiff", filenames_1)

for(i in 1:45){
  tiff(paste("Q:/Research/EXTERNAL PARTNERS/NCAR/Elevation_changes/",filenames_1tiff[i],sep=""), 
  ...
  print(ggplot(signsps_1[[i]][signsps_1[[i]]$year>1850&
                                ...
                                signsps_1[[i]]$minimumElevationInMeters>0,], aes(year,minimumElevationInMeters))+
          geom_point(shape=1, size=0.5)+
          stat_smooth(method="lm")+
          theme(axis.text=element_text(size=5),
                axis.title=element_text(size=7))+
...
nrow(all_sum) #339
nrow(all_sum[all_sum$Pr...t..<0.05& all_sum$Estimate<0,]) #11
mean(all_sum$Estimate[all_sum$Pr...t..<0.05 & all_sum$Estimate<0]) #-2.715357
mean(all_sum$sp.adj.r.squared[all_sum$Pr...t..<0.05 & all_sum$Estimate<0]) #-0.06469807

nrow(all_sum[all_sum$Pr...t..<0.05 & all_sum$Estimate>=0,]) #115
mean(all_sum$Estimate[all_sum$Pr...t..<0.05 & all_sum$Estimate>=0]) #2.625029
mean(all_sum$sp.adj.r.squared[all_sum$Pr...t..<0.05 & all_sum$Estimate>=0]) #0.04530414
