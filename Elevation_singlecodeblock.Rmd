...
# split by elevational group
table(bloom.sp$elevation)

bloom.sp$BinElevation <- cut(bloom.sp$elevation,
                             breaks=seq(3200, 4400, 100),
                             labels=FALSE)


# Just the first collection per species per year 
elevation.earl.sp.yr <- do.call(rbind,
                      lapply(split(bloom.sp, list(bloom.sp$ID,
                                                  bloom.sp$BinElevation)),
  function(id){
    if(length(id$Year)>0){
      id <- id[order(id$Year),]
      id$DataYrs <- nrow(id[!duplicated(id$Year) &
                              !is.na(id$Year),])
      rows <- aggregate(startDayOfYear~Year, id,
                        FUN=min)
      out <- merge(id, rows, by=c("startDayOfYear","Year"))
      out <- out[order(out$Year),]
      out <- out[!duplicated(out$Year),]
...
#### START AFTER HERE ####
elevation.sp.10plus <- elevation.earl.sp.yr[elevation.earl.sp.yr$DataYrs>=10,] 
length(unique(elevation.sp.10plus$ID))#230
head(elevation.sp.10plus)

elevation.merge.name.accpt <- merge(elevation.sp.10plus, synonyms.plantsyn.accpt, by = "ID") 
length(unique(elevation.merge.name.accpt$ID))#219 (said 45 rows are identical??)
...
```{r}
synonyms.plantsyn[grep("Cerastium bee", synonyms.plantsyn$SYNONYMS),]
