...
yrswithdata <- lapply(unique(datatbl$Var1), function(x){
  length(datatbl$Var2[datatbl$Var1 == x & datatbl$Freq > 0])
}) 
...
nrow(hdd.bl[hdd.bl$p.value < 0.05 & hdd.bl$Slope > 0,]) #278
mean(hdd.bl$Slope[hdd.bl$p.value < 0.05 & hdd.bl$Slope > 0])
mean(hdd.bl$adj.r.squ[hdd.bl$p.value < 0.05 & hdd.bl$Slope > 0])
...
npn.specimens <- merge(min.dates,npn.sp, by = "SciName")
nrow(npn.specimens)
nrow(min.dates)
nrow(npn.sp)
...
gsub("(.*)?(.{4}).*$", "\\1", spec.sps) #removed the last three... closer
firstword <- sub("(?<=.{3})(.)*", "", spec.sps, perl = TRUE) 
sub("( .*)(?<=.{3})(.)*", "", spec.sps, perl = TRUE) # everyting up to the first space
...
spec.sps[grep("coerulea", spec.sps)]
min.dates$Scientific.Name <- gsub("coerulea", "caerulea", min.dates$Scientific.Name)
