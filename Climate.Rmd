...
npn.only[,-c(3,13,length(npn.only))] <- sapply(npn.only[,-c(3,13,length(npn.only))], 
...
ggplot(npn.only, aes(Year,startDayOfYear,colour=SciName))+
  geom_point()+
  stat_smooth(method="lm")+
  theme_bw()+
  labs(title="Earliest Bloom Date by Year")+
...
yr.npn[yr.npn$p.value<0.05,]


hi.npn <- JulBloomDate.subset("Avg_Hi", npn.only) #mostly positive slope
hi.npn[hi.npn$p.value<0.05,]
colMeans(hi.npn[hi.npn$p.value<0.05,-9])


lo.npn <- JulBloomDate.subset("Avg_Lo", npn.only) #mostly positive slope
lo.npn[lo.npn$p.value<0.05,]
...
colMeans(precip.npn[precip.npn$p.value<0.05,-9])
...
#Repeated Measures. Repeated in this case is year
#wants an ID for the repeated measures at each point, we have one measure per
```{r}
summary(aov(GDD~factor(Decade)*factor(Day)+Error(factor(Year)),data=meltd))

# make sure it's equal sample sizes just get rid of 2011...
meltd.sym <- meltd[meltd$Year != 31,]
summary(aov(GDD~factor(Decade)*factor(Day)+Error(factor(Year)),data=meltd.sym))

summary(aov(GDD~Year*Day+Error(Year), data=meltd))
```
