...
dayxyear <- summary(lm(startDayOfYear~Year, data=merge.name.accpt))
dayxyear$coefficients[2,1]*61
```
...
```{r}
dpi = 1200
tiff("Q:/Research/Projects/alpine-phenology/Fig. 4_fm.tiff", width = 174, height = 150, compression="lzw", units = "mm", res=dpi)

ggplot() +
  geom_point(data=merge.name.accpt, aes(Year, startDayOfYear), shape=17, 
             colour="gray70")+
  geom_point(data=merge.name.accpt.ALL, aes(Year, startDayOfYear), shape=".",
             colour="black")+
...
write.table(yr.npn_2, file = "Q:/Research/Projects/alpine-phenology/phenNPN_yr_1???8.csv", sep=",")
