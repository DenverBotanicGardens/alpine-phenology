...
ggplot(merge.name.accpt, aes(Year, startDayOfYear))+
  geom_point()+
  geom_line(data=dat.first.multiple)+
  theme_bw()
```
...
#not right, ID treated like value, not categorical
lm.ALL.sp <- lm(startDayOfYear~Year+factor(ID), data=merge.name.accpt.ALL)
seg.ALL.sp <- segmented(lm.ALL.sp, seg.Z = ~Year,
                     psi = list(Year=1990))
...
Segmented analyses - go find them
```{r}
