...
allcol <- aggregate(ID~Year, data = bloom.sp, function(x) length(unique(x)))
...
#n.dups <- aggregate(ID~Year+elevation+Decimal.Latitude+Decimal.Longitude+scientificName, 
#                    function(x) length(x), data = bloom.sp2)
...
#earlier
nrow(yr.1950_2.ALL[yr.1950_2.ALL$p.value<0.05 & yr.1950_2.ALL$Slope<0,]) #45->35
...
trianges gray (2) = just first
black dots (19) = all, for mean so should never be only an open triangle but yes should be some spare dots
```{r}
tiff("Q:/Research/Projects/alpine-phenology/Fig. 4_firstandmeanflowering.tiff", width = 174, height = 150, compression="lzw", units = "mm", res=dpi)

ggplot() +
  geom_point(data=merge.name.accpt, aes(Year, startDayOfYear), shape=2, 
             colour="darkgray", alpha=0.75)+
  stat_smooth(data=merge.name.accpt, aes(Year, startDayOfYear),
              method="lm",colour = "gray30", size = 0.75, linetype=5)+
  geom_point(data=merge.name.accpt.ALL, aes(Year, startDayOfYear), shape=20, alpha=0.75)+
