...
# Or an NPN list that Becky pulled down, instead of the other one that Mary got
npn_0303 <- read.csv("Q:/Research/Projects/alpine-phenology/NPN spp download 20170303/status_intensity_observation_data.csv")
npn_0303$SciName <- apply(npn_0303[,c("Genus","Species")], 1,
                         function(x) paste(x[1],x[2], collapse=" "))


#although lots of these might not have 10 years    
#Match names - synonyms and all
npn_0303npn.big2 <- intersect(npn_0303$SciName, merge.name.accpt$scientificName)
npn_0303npn.big <- intersect(npn_0303$SciName, bloom2$scientificName) 
length(npn_0303npn.big2) #22
length(npn_0303npn.big) #21
