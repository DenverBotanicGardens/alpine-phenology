...
## snow2 is from Results_medl    
#### snow2 is from snow.total which is row sums of avg.p.yr
#### avg.p.yr is from 'snow'
...
```{r}
snow.foo <- merge(min1950GDD_2_10,snow2, by.x="Year",by.y="year")
head(snow.foo)
sort(unique(snow.foo$Year))
```

# From Results_medl, line 163
```{r}

sp.yr <- as.data.frame(table(bloom2$Year[bloom2$Year >= 1979],
                             bloom2$scientificName[bloom2$Year >= 1979]))
...
spyears.snow <- do.call(rbind,lapply(bysp, function(x){
  out <- data.frame(DataYrs = sum(x$Freq), Species = unique(x$Var2))
  out
}))
sp.10plussnow <- spyears.snow[spyears.snow$DataYrs >=10,]
nrow(sp.10plussnow) #421

max.swe <- aggregate(melt.avg.swe$value, by=list(melt.avg.swe$Var1), max)
names(max.swe) <- c("Year","SWE(mm)")
snow.1950 <- merge(min1950_2,max.swe, by="Year")
#Merge with only species that have 10+ years from 1978:2011
snow.1950 <- merge(sp.10plussnow, snow.1950, by.y = "scientificName", by.x = "Species")

JulBloomDate.min1950 <- function(variable,dataset){
  namesID <- unique(dataset$Species)
  colnum <- match(variable,names(dataset))
  JYC <- lapply(namesID, function(x){
    blnew <- dataset[dataset$Species == x,]
    foo1 <- summary(lm(blnew$startDayOfYear~blnew[,colnum]))
    jul.yr.coef <- data.frame(cbind(data.frame(t(foo1$coefficients[2,])),
                                    foo1$coefficients[1,1],
                                    t(foo1$fstatistic),
...
snow.table <- JulBloomDate.min1950("SWE(mm)",snow.1950)

snow.table$genus <- gsub(" .*$", "", snow.table$Species)
m15 <- merge(snow.table,asuFams,by="genus")
nrow(m15) #545 

snow.table[!complete.cases(snow.table),] #Kobresia schoenoides
spyears[grep("Erysimum", spyears$Species),]
spyears[grep("Kobresia", spyears$Species),]

write.table(m15, file = "Q:/Research/Projects/alpine-phenology/phensum_MaxSnowWaterEqu.csv", sep=",")

#earlier
nrow(snow.table[snow.table$p.value<0.05 & snow.table$Slope<0,]) #
mean(snow.table$Slope[snow.table$p.value<0.05 & snow.table$Slope<0]) 
mean(snow.table$adj.r.squ[snow.table$p.value<0.05 & snow.table$Slope<0])

#later
nrow(snow.table[snow.table$p.value<0.05 & snow.table$Slope>=0,]) #5
mean(snow.table$Slope[snow.table$p.value<0.05 & snow.table$Slope>=0])
mean(snow.table$adj.r.squ[snow.table$p.value<0.05 & snow.table$Slope>=0])
...
```{r}
ggplot(unique(snow.foo[,c(5,length(snow.foo))]), aes(Raw_Precip,depth ))+
  stat_smooth(method="lm")+
  geom_point()


snow.precip <- merge(meanppt, snow2, by.x="Year",by.y="year") 

ggplot(snow.precip, aes(data,depth))+
  stat_smooth(method="lm")+
  geom_point()

cor(snow.precip$depth,snow.precip$data) #0.13 not well correlated
cor.test(snow.precip$depth,snow.precip$data) # not significantly correlated!

```


Snow correlated to Raw_Precip
```{r}
raw.precip.snow <- merge(unbloom, snow2, by.x = "Year", by.y="year")

cor(raw.precip.snow[,-1])
cor.test(raw.precip.snow$Raw_Precip,raw.precip.snow$depth)
cor.test(raw.precip.snow$Avg_Lo,raw.precip.snow$depth)
cor.test(raw.precip.snow$Avg_Hi,raw.precip.snow$depth)

pairs(raw.precip.snow[,-1])


# But outlier might be messing it up!
dat <- data.frame(X = raw.precip.snow$Raw_Precip, Y = raw.precip.snow$depth)
dat <- dat[dat$X != 55443,]
pairs(dat)
cor.test(dat$X,dat$Y)

ggplot(raw.precip.snow, aes(Raw_Precip, depth))+
  stat_smooth(method="lm")+
  geom_point()
```
...
test1 <- lmer(startDayOfYear ~ Raw_Precip + Avg_Hi + Avg_Lo +
               Raw_Precip:Avg_Hi + Raw_Precip:Avg_Lo + Avg_Hi:Avg_Lo +
                Raw_Precip:Avg_Hi:Avg_Lo +
                (1|scientificName), data = min1950_2, REML=FALSE)

summary(test1)

anova(test1, ddf="Satterthwaite", type=3)

test.status <- glht(test1, linfct=mcp(status="Tukey"))

```

 
Park and Schwartz 2015, differing responses for early, mid, and late blooming species   
```{r}
# This doesn't break it down by species, might take away by species assessments
eml <- min1950_2
head(eml)
hist(eml$startDayOfYear)
# arbitrarily split <175, middle, >225
eml$season <- "middle"
eml$season[eml$startDayOfYear < 175] <- "early"
eml$season[eml$startDayOfYear > 225] <- "late"

ggplot(eml, aes(Avg_Hi, startDayOfYear, colour = scientificName))+
  geom_point()+
  stat_smooth(method="lm")+
  facet_grid(~season)+
  theme_bw()+
...
summary(lm(startDayOfYear~Avg_Hi*season, data=eml))
```
 
 
 # shouldn't need the following fixes after using synonymy from our database...
```{r, eval=FALSE}
# accepted names http://cumuseum-archive.colorado.edu/Research/Botany/Databases/vascular_plants.pdf
#found Boechera drummondii and Arabis drummondii   
# Boechera laevigata isn't a species
unique(bloom2$scientificName[grep("Boechera",bloom2$scientificName)])
unique(bloom2$scientificName[grep("Arabis",bloom2$scientificName)])
unique(bloom2$scientificName[grep("Turritis",bloom2$scientificName)])


bloom2 <- as.data.frame(sapply(bloom2, gsub,
                               pattern = "Arabis oxylobula",
                               replacement = "Boechera oxylobula"))

bloom2 <- as.data.frame(sapply(bloom2, gsub,
                               pattern = "Arabis demissa var. demissa",
                               replacement = "Boechera oxylobula"))

bloom2 <- as.data.frame(sapply(bloom2, gsub,
                               pattern = "Arabis laevigata",
                               replacement = "Boechera laevigata"))

bloom2 <- as.data.frame(sapply(bloom2, gsub,
                               pattern = "Arabis divaricarpa",
                               replacement = "Boechera divaricarpa"))

bloom2 <- as.data.frame(sapply(bloom2, gsub,
                               pattern = "Arabis drummondii",
                               replacement = "Boechera drummondii"))

bloom2 <- as.data.frame(sapply(bloom2, gsub,
                               pattern = "Arabis lemmonii",
                               replacement = "Boechera lemmonii"))
...
tonum <- c(3:length(bloom2))
bloom2[,tonum] <- sapply(bloom2[,tonum], function(num){
