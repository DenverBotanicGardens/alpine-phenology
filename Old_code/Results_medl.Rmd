...
```{r}
long.avg.snow$Snow.Depthcm <- 2.54*long.avg.snow$Snow.Depth
long.avg.snow$Month <- gsub("in", "cm", long.avg.snow$Month)

long.avg.snow$Month <- factor(long.avg.snow$Month, levels=unique(long.avg.snow$Month))
```
```{r}
ggplot(long.avg.snow, aes(Year, Snow.Depthcm))+ 
  geom_line()+
...
summary(lm(Snow.Depth~Year*Month, data=long.avg.snow))
```
...
Check how much data for snow water equivalent
```{r}
avg.swe <- do.call(rbind, lapply(split(snow[,grep("_Snow Water", names(snow))],
                                       snow$'Water Year_'),
                                 function(x) colMeans(x, na.rm = TRUE)))
melt.avg.swe <- melt(avg.swe)

ggplot(melt.avg.swe, aes(as.numeric(Var2),value))+
  geom_point()+
  geom_smooth()+
  facet_wrap(~Var1)


Maxpyr <- aggregate(melt.avg.swe$value, by=list(melt.avg.swe$Var1), which.max)
sort(Maxpyr$Group.1)
mean(Maxpyr$x)
unique(melt.avg.swe$Var2)[7] #April is average peak 
summary(lm(x~Group.1, data=Maxpyr)) # peak is getting slightly earlier
```
...
#snow depth signficnatly decreasing over this time
...
bloom2[grep("Eritri", bloom2$scientificName),]

asuFams[grep("Polem", asuFams$genus),]
asuFams <- rbind(asuFams, c("Boraginaceae","Eritrichium",))
